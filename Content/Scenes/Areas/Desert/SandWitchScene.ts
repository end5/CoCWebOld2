...
export function encounter(player: Character): NextScreenChoices {
    ...
    if (ExgartuanFlags.LOCATION === 1 && player.body.cocks.get(0)!.area > 100 && ExgartuanFlags.SLEEP_COUNTER === 0) {
        CView.text("\n\nThe " + player.inventory.armor.displayName + " covering your lower half hits the ground, as if yanked down by magic.  Your " + describeCock(player, player.body.cocks.get(0)) + " pulsates darkly, growing rigid in seconds as the demon within you takes over.  It barks, \"<i>Fuck, how about I cast my spell on you baby?</i>\"\n\n");
        CView.text("The sandwitch ");
        if (player.stats.cor < 50)
            CView.text("and you both turn crimson");
        else CView.text("turns crimson");
        CView.text(" as you yank your " + player.inventory.armor.displayName + " back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!");
        return CombatManager.beginBattle(new Encounter(player, new SandWitch()));
    }
...
function allowSandWitchMagic(player: Character): NextScreenChoices {
    ...
    if (player.body.hair.color === "sandy blonde") {
        ...
        const largestRow = player.body.chest.sort(BreastRow.Largest).get(0)!;
        if (player.body.chest.length === 0 || largestRow.rating === 0) {
        ...
        if (largestRow.rating >= 1 && largestRow.rating <= 2) {
        ...
        if (firstRow.nipples.count < 1) {
            ...
            firstRow.nipples.count = 1;
            player.stats.sens += 2;
            player.stats.lust += 1;

        }
        const mostLac = player.body.chest.sort(BreastRow.LactationMost).get(0)!;
        if (mostLac.lactationMultiplier > 0) {
            CView.text("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ");
            if (mostLac.lactationMultiplier < 2)
            ...
            if (mostLac.lactationMultiplier >= 2 && mostLac.lactationMultiplier <= 2.6)
            ...
            if (mostLac.lactationMultiplier > 2.6 && mostLac.lactationMultiplier < 3)
            ...
            if (mostLac.lactationMultiplier >= 3)
...
export function sandwitchRape(player: Character): NextScreenChoices {
    ...
    if (player.stats.lust > 99) {
        ...
        player.orgasm();
        CView.clear();
        CView.text("Your wobbly legs give out underneath you as your body's will to fight evaporates.\n\n");
        if (player.body.hair.color.indexOf("sandy blonde") !== -1) {
            ...
            const firstRow = player.body.chest.get(0)!;
            if (player.body.chest.length === 0 || player.body.chest.sort(BreastRow.Largest).get(0)!.rating === 0) {
                CView.text("You grow a perfectly rounded pair of C-cup breasts!  ");
                if (player.body.chest.length === 0) player.body.chest.add(new BreastRow());
                firstRow.count = 2;
                firstRow.rating = 3;
            ...
            if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating > 0 && player.body.chest.sort(BreastRow.Largest).get(0)!.rating < 3) {
                CView.text("Your breasts suddenly balloon outwards, stopping as they reach a perfectly rounded C-cup.  ");
                firstRow.rating = 3;
                player.stats.sens += 1;
                player.stats.lust += 1;
            ...
            if (player.body.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) < 1) {
                ...
                while (temp > 0) {
                    ...
                    if (firstRow.nipples.count < 1) player.body.chest.get(0)!.nipples.length = .2;
                    firstRow.nipples.count = 1;
                }
                player.stats.sens += 2;
                player.stats.lust += 1;
            ...
            if (lacMost.lactationMultiplier > 0) {
                ...
                if (lacMost.lactationMultiplier < 3) {
...
function sandWitchBadEndPartThree(player: Character): NextScreenChoices {
    ...
    if (player.effects.has(EffectType.MarblesMilk)) {
...
function sandwitchRaped(player: Character, sandWitch: Character): NextScreenChoices {
    ...
    if (player.gender === 1 || player.gender === 3) {
        ...
        if (player.stats.cor < 50) {
            ...
            return { choices: [["Breasts", sandwitchBewbs], ["Sex", sandwitchSex]] };
        }
        // Really corrupt
        else {
            // Single cock
            if (player.body.cocks.length === 1) {
                ...
                if (player.body.cocks.filter(Cock.FilterType(CockType.DOG)).length > 0 && player.body.cocks.get(0)!.knotMultiplier > 1.4) knotSandwitch(player);
                    ...
                    CView.text("You place your " + describeFoot(player) + " in the wet sand, pulling her hair and making her look you in the eyes. The sand witch looks shocked at your display of power, only turning you on more. You undo your " + player.inventory.armor.displayName + " and grope her nethers, only to find a surprise. She has " + numToCardinalText(player.body.cocks.length + 1) + " pussies and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping head of your " + describeCock(player, player.body.cocks.get(0)) + " to one of her nethers, slowly easing into her depths.  ");
...
function sandwitchCentaurBoning(player: Character, sandWitch: Character) {
    CView.sprite(SpriteName.SandWitch); // 50;
    player.orgasm();
    CView.text("The Sand Witch is panting before you, her clothes in disarray ");
    // [win via HP]
    if (sandWitch.stats.HP < 1) CView.text("and torn in many places. ");
    // [win via lust]
    ...
    if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating > 3) {
        ...
        // [win via HP]
        if (sandWitch.stats.HP < 1) CView.text("is unable to stop herself from slowly licking her lips. ");
        // [win via lust]
    ...
    if (player.stats.cor < 70) {
        ...
        // [has breasts]
        if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating > 0) CView.text("; your " + describeNipple(player, player.body.chest.get(0)) + "s  become stiff");
        // [cock only]
        ...
        // [has breasts]
        if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating > 0) CView.text("fondle your " + describeAllBreasts(player) + " and ");
        ...
        if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating > 0) {
            ...
            if (player.body.chest.sort(BreastRow.LactationMost).get(0)!.lactationMultiplier >= 1) {
                ...
                // [light-med lactation]
                if (player.body.chest.sort(BreastRow.LactationMost).get(0)!.lactationMultiplier < 3) CView.text("small streams running out ");
                // [heavy lactation]
...
export function beatSandwitch(player: Character, sandWitch: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50;
    CView.clear();
    if (sandWitch.stats.lust > 99)
        CView.text("You smile in satisfaction as the " + sandWitch.desc.name + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the horny witch...\n\nDo you rape her?");
    else
        CView.text("You smile in satisfaction as the " + sandWitch.desc.name + " drops down on all fours and struggles to rise.  Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...");
...
function missingoSex5(player: Character): NextScreenChoices {
    ...
    CView.text("Yeah no, fuck this shit. Seriously. You grip your controller with a barely restrained fury, fighting the urge to throw it across the room in a fit. Taking a moment, you collect yourself, and test the game. Yep, totally unresponsive to your buttons now. Damn thing froze AGAIN.\n\n");
...
function missingoSex6(player: Character): NextScreenChoices {
    ...
    CView.text("You successfully restore your game. Everything's intact. At least, you thought it was. You must have messed up the password somewhere along the line, but oddly enough, the only difference is the clock being set an hour ahead and a few extra gems and EXP in your pocket.\n\n");
...
function ovipositSandWitches(player: Character, sandWitch: Character): NextScreenChoices {
    CView.clear();
    CView.text("As you glance down at the ");
    if (sandWitch.stats.HP < 1) CView.text("bruised and battered");
    ...
    if (sandWitch.stats.HP < 1) CView.text("fright as she clamors to get away.");
    else CView.text("an uneasy curiosity to see what exactly you have planned.");

    // PC won through HP victory:
    if (sandWitch.stats.HP < 1) CView.text("\n\nRolling your eyes, you offer a sympathetic hand to the defeated witch, showing her that you don't mean to hurt her any further, that you have something more... pleasurable in mind.  "); CView.text("The sand witch slows to a dead stop as she assesses your intentions, which are made all the more clear as you disrobe and toss your [armor] aside, exposing your ");
...
export function witchBirfsSomeBees(player: Character): NextScreenChoices {
    ...
    if (Settings.sillyMode) CView.text(", echoing tiny little cries of \"<i>@</i>\"");
...
function sandwitchSpanking(player: Character, sandWitch: Character): NextScreenChoices {
    CView.clear();
    ...
    if (sandWitch.stats.HP < 1) CView.text("  You can't even stand up!");
    ...
    if (sandWitch.stats.lust > 99) CView.text(" but still masturbating");
    ...
    if (player.effects.has(EffectType.FireLord) || player.effects.has(EffectType.Hellfire) || player.effects.has(EffectType.Dragonfire)) CView.text(", breathing out a gout of flame in rage, but you can't even direct the blast properly like this.  All it does is melt some of the sand below into glass");
    ...
    sandWitch.stats.lust = 98;
    sandWitch.stats.HP = 2;
    player.stats.lust = 100;
    SandWitchFlags.COMBAT_BONUS_XP_VALUE = sandWitch.stats.XP;
    player.orgasm();
    player.stats.sens += 5;
