...
class Entwine extends CombatAction {
    ...
    public useAction(char: Character, enemy: Character) {
        ...
        char.effects.create(EffectType.PCTailTangle, { counter: 0 });
...
class Illusion extends CombatAction {
    ...
    public useAction(char: Character, enemy: Character) {
        ...
        if (randInt(100) < resist) {
            ...
            char.effects.create(EffectType.Illusion, { spe: 20 });
        }
    }
}
...
class Seal extends CombatAction {
    ...
    public useAction(char: Character, enemy: Character) {
        ...
        if (select === 0) {
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.Attack });
        }
        else if (select === 1) {
            // Tease:
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.Tease });
        }
        // Spells:
        else if (select === 2) {
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.Spells });
        }
        // Items:
        else if (select === 3) {
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.Items });
        }
        // Run:
        else if (select === 4) {
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.MoveAway });
        }
        // P.Special:
        else if (select === 5) {
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.PhysSpec });
        }
        // M.Special:
        else {
            ...
            enemy.effects.create(EffectType.Sealed, { combatExpire: 4, blockedTypes: CombatActionType.MagicSpec });
        }
...
KitsuneReactions.set("Struggle", {
    beforeUseAction: (kitsune: Character, enemy: Character) => {
        const pcTailTangle = kitsune.effects.getByName(EffectType.PCTailTangle);
        if (!pcTailTangle || !pcTailTangle.values.counter) return false;
        CView.clear();
        // Struggle:
        ...
        // Success
        if (randInt(20) + enemy.stats.str / 20 + pcTailTangle.values.counter >= 12) {
