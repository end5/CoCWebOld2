...
import { camp, sleepWrapper, sleepRecovery } from 'Content/Scenes/Camp';
import { Cock, CockType } from 'Engine/Body/Cock';
import { describeCocksLight, describeCock, describeCockHead } from 'Content/Descriptors/CockDescriptor';
...
export const ArianFlags = Flags.register("Arian", {
    ARIAN_EGG_COUNTER: 0,
    ARIAN_VAGINA: 0,
    ARIAN_EGG_EVENT: 0,
    ARIAN_LESSONS: 0,
...
export function meetArian(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ...
            ["Don't Help", choiceWrap(dontHelpArianWhenYouMeetHim)],
            ["Never Help", choiceWrap(dontHelpArianWhenYouMeetHim, true)],
        ]
    };
}
...
function arianHomeMenu(player: Character): NextScreenChoices {
    ...
    // If no story dialogue
        ...
        if (ArianFlags.ARIAN_S_DIALOGUE >= 2) choices[1] = ["Sex", choiceWrap(arianSexMenu)];
        ...
        if (Time.hour >= 17 && arianFollower()) choices[8] = ["Sleep With", choiceWrap(sleepWithArian, true)];
        if (PlayerFlags.SLEEP_WITH === "Arian") choices[8] = ["NoSleepWith", dontSleepWithArian];
        if (!arianFollower()) choices[9] = ["Back", telAdreMenu];
        else choices[9] = ["Back", camp.campLoversMenu];
    }
...
function bargeInOnArian(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0 && player.body.vaginas.length <= 0) {
        ...
        choices[0] = ["Don't Mind", youDontMindBeingGayForArian];
        choices[1] = ["Like Girls", youLikeGirlsNotSickLizardDudes];
    }
    ...
    // (else if PC is female)
        ...
        choices[0] = ["Next", getPenetratedByArianAndHisHitlerMustache];
    }
    // (else) //if PC is a herm
        ...
        choices[0] = ["Like Male", hermsLikeMaleArian];
        choices[1] = ["Like Female", hermsLikeFemaleArian];
    }
...
function arianStoryDialogue1(player: Character): NextScreenChoices {
    ...
    // PC has met Marae:
    if (player.effects.has(EffectType.MetMarae)) CView.text("Marae herself told you they showed up about, what, 20-30 years ago?");
...
function arianPlot5(player: Character): NextScreenChoices {
    ...
    if (camp.companionsCount() > 1) CView.text("  There are other people living with you already, so what's one more?  Right?");
    CView.text("</i>\"");
...
function arianMagicLessons(player: Character): NextScreenChoices {
    ...
    if (player.stats.int < 25) {
        ...
        if (!arianFollower()) CView.text("make your way back to your camp.");
    ...
    // (else if PC int < 50 //Teach Charge Weapon if PC doesn't know.
        ...
        if (!arianFollower()) CView.text(", making your way back to camp.");
        else CView.text(", exiting [Arian eir] tent and going about your business.");

        // (if PC doesn't know Charge Weapon)
        if (!player.effects.has(EffectType.KnowsCharge)) {
            CView.text("\n\nAs you ");
        ...
        if (!player.effects.has(EffectType.KnowsBlind)) {
        ...
        if (!player.effects.has(EffectType.KnowsWhitefire)) {
...
function sexTalkFinish(player: Character, newl: boolean = false): NextScreenChoices {
    if (newl) CView.clear();
    else CView.text("\n\n");
    CView.text("Satisfied with your little chat, you pat the lizan's head and excuse yourself, heading back to camp.");
    return { next: passTime(1) };
}
...
function takeYerLizardHomePartII(player: Character): NextScreenChoices {
    ...
    if (camp.followersCount() > 1) CView.text(" and your other companions");
    CView.text(".");
...
function arianSexMenu(player: Character, output: boolean = true): NextScreenChoices {
    if (output) {
        CView.clear();
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.cocks.find(Cock.CockThatFits(50)) && player.body.cocks.filter(Cock.CocksThatFit(50)).get(1) && ArianFlags.ARIAN_VAGINA > 0) choices[8] = ["Double Pen", doublePenetrateArian];
        // Docking
        // ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.)
        // PC cock area must be <= 30.
        if (player.body.cocks.sort(Cock.Smallest).get(0)!.area <= 30 && ArianFlags.ARIAN_COCK_SIZE < 3 && ArianFlags.ARIAN_COCK_SIZE > 0) choices[7] = ["Docking", arianDocking];
    }
    // Get Anal:
...
function giveArianAnal(player: Character): NextScreenChoices {
    ...
    // Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
        ...
        if (ArianFlags.ARIAN_ASS_CHAT === 0) {
            ...
            if (!arianFollower()) CView.text("back to Tel'Adre's streets, and from there back to camp.");
...
function doublePenetrateArian(player: Character): NextScreenChoices {
    ...
    const othercockTHatFits = player.body.cocks.filter(Cock.CocksThatFit(ArianFlags.ARIAN_CAPACITY)).get(1);
    ...
    CView.text("\n\nArian's ass goes slack around your " + describeCock(player, othercockTHatFits) + ", and Arian slowly slides off your shaft to plop on her bed; eyes closed in bliss, as she takes a short nap.");
...
function giveIncubusDraftToArian(player: Character): NextScreenChoices {
    ...
    if (ArianFlags.ARIAN_BREASTS > 1 && (!Settings.hyperHappy)) {
    ...
    else if (ArianFlags.ARIAN_BREASTS === 1 && (!Settings.hyperHappy)) { // Now you are male again.
...
function succubiMilkForArian(player: Character): NextScreenChoices {
    ...
    else if (ArianFlags.ARIAN_COCK_SIZE > 1 && (!Settings.hyperHappy)) {
    ...
    else if (ArianFlags.ARIAN_COCK_SIZE === 1 && ArianFlags.ARIAN_DOUBLE_COCK >= 1 && (!Settings.hyperHappy)) {
...
function imbueTalisman(player: Character): NextScreenChoices {
    ...
    */
    ...
    const choices: ScreenChoice[] = [];
    if (player.inventory.items.reduce(ItemStack.TotalQuantityOf(MaterialName.BlackChitin), 0) >= 2 && player.inventory.items.has(MaterialName.ToughSpiderSilk))
        choices[0] = ["Shielding", choiceWrap(arianSpellPlace, "Shielding Spell")];
    if (player.inventory.items.reduce(ItemStack.TotalQuantityOf(ConsumableName.GoblinAle), 0) >= 2 && player.inventory.items.has(ConsumableName.SweetGossamer))
        choices[1] = ["Immolation", choiceWrap(arianSpellPlace, "Immolation Spell")];
    choices[9] = ["Back", arianHomeMenu];
...
function arianSpellPlace(player: Character, spell: string): NextScreenChoices {
    ...
    if (spell === "Shielding Spell") {
        player.effects.create(EffectType.ShieldingSpell);
        // Shielding Spell: 2x Black Chitin and 1x Tough Silk. - Increases defense for the duration of the battle.
    ...
    if (spell === "Immolation Spell") {
        player.effects.create(EffectType.ImmolationSpell);
        // Immolation Spell: 2x Goblin Ale and 1x Sweet Gossamer. - Deals damage over time.
...
function clearCharges(player: Character) {
    if (player.effects.has(EffectType.ShieldingSpell)) player.effects.removeByName(EffectType.ShieldingSpell);
    if (player.effects.has(EffectType.ImmolationSpell)) player.effects.removeByName(EffectType.ImmolationSpell);
}
...
// Sleep With Arian
export function sleepWithArian(player: Character, newl: boolean = false): NextScreenChoices {
    if (newl) CView.clear();
    PlayerFlags.SLEEP_WITH = "Arian";
    ...
    if (ArianFlags.ARIAN_ANAL_XP < 33) {
        ...
        return {
            choices: [
                ["Listen", listenToLowAnalXPArian],
                ["Sleep", dontListenToLowAnalXPArian],
            ]
        };
    }
    // (else if AnalXP <66)
        ...
        return {
            choices: [
                ["Listen", listenToMediumAnalXPArian],
                ["Sleep", dontListenToMediumAnalXPArian],
            ]
        };
    }
    else { // AnalXP <= 100
        ...
        return {
            choices: [
                ["Tease", TeaseHighAnalXPArian],
                ["Sleep", dontTeaseHighAnalXPArian],
            ]
        };
    }
}
...
export function arianEggingEvent(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ["Brown", choiceWrap(pickAnEggArian, "brown")],
            ["Purple", choiceWrap(pickAnEggArian, "purple")],
            ["Blue", choiceWrap(pickAnEggArian, "blue")],
            ["Pink", choiceWrap(pickAnEggArian, "pink")],
            ["White", choiceWrap(pickAnEggArian, "white")],
            ["Rubber", choiceWrap(pickAnEggArian, "rubbery black")],
            ["No Thanks", leaveEggs],
        ]
    };
}
...
export function arianLaysEggs(player: Character): NextScreenChoices {
    ...
    let itype;
    if (ArianFlags.ARIAN_EGG_COLOR === "brown") itype = ConsumableName.LargeEggBrown;
    else if (ArianFlags.ARIAN_EGG_COLOR === "purple") itype = ConsumableName.LargeEggPurple;
    else if (ArianFlags.ARIAN_EGG_COLOR === "blue") itype = ConsumableName.LargeEggBlue;
    else if (ArianFlags.ARIAN_EGG_COLOR === "pink") itype = ConsumableName.LargeEggPink;
    else if (ArianFlags.ARIAN_EGG_COLOR === "white") itype = ConsumableName.LargeEggWhite;
    else if (ArianFlags.ARIAN_EGG_COLOR === "rubbery black") itype = ConsumableName.LargeEggBlack;
    else itype = ConsumableName.LargeEggBrown;
