...
export const MilkWaifuFlags = Flags.register("MilkWaifu", {
    MILK_NAME: '',
...
export function milkSlave(): boolean {
    return MilkWaifuFlags.MILK_NAME.length > 0;
}
...
export function arriveWithLacticWaifuAtCamp(player: Character): NextScreenChoices {
    ...
    return { next: choiceWrap(nameZeMilkBath, nameField) };
}

function nameZeMilkBath(player: Character, nameField: InputElement): NextScreenChoices {
    if (nameField.value === "" || typeof nameField.value === 'number') {
        CView.clear();
        CView.text("<b>You must give her a name.</b>");

        nameField.value = "Bath Slut";
        return { next: choiceWrap(nameZeMilkBath, nameField) };
    }

    MilkWaifuFlags.MILK_NAME = nameField.value;
    CView.clear();

    // Call her Bath Slut (You Asshole)
    if (MilkWaifuFlags.MILK_NAME === "Bath Slut") CView.text("Fuck it, Bath Slut it is.  At least she won't get confused.");
    else if (MilkWaifuFlags.MILK_NAME === "Biscuit") CView.text("Fuck it, you may as well butter her buns!");
    ...
    // if (player.effects.has(EffectType.CampRathazul) && player.effects.getByName(EffectType.MetRathazul).value2 >= 4) {
...
//     if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 0) choices[9] = ["Back", camp.campSlavesMenu];
//     if (MilkWaifuFlags.FOLLOWER_AT_FARM_BATH_GIRL === 1) choices[9] = ["Back", farm.farmCorruption.rootScene];
...
//         }
//         else {
//             if (farm.farmCorruption.numTattoos("milky") > 1) CView.text("She has the following tattoos emblazoned across her body:\n");
...
//             if (player.effects.has(EffectType.CampMarble)) CView.text(" and Marble");
//             CView.text(" pause");
//         }
//         else if (player.effects.has(EffectType.CampMarble)) CView.text(", enough to give Marble pause");
...
//     if (player.effects.has(EffectType.PureCampJojo)) count++;
...
//     // (+Lust, -Fatigue)
//     player.stats.raw.lust += 10 + player.stats.sens / 10;
...
//     if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 7) CView.text(", pressing deep into your own huge rack, your nipples flicking across her own leaky tits");
//     CView.text(".");
...
//     if (player.body.vaginas.length > 0 && player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 3) choices[1] = ["LesboFuck", beARugMunchingMilkDyke];
...
//         // If PC has Isabella:
//         if (isabellaFollowerScene.isabellaFollower()) {
//             if (isabellaFollowerScene.isabellaAccent()) CView.text("\n\n\"<i>Tsk, [name].  Vhat are you doing to zhat poor voman?");
...
//             if (isabellaFollowerScene.isabellaAccent()) CView.text("zing");
//             else CView.text("thing");
//             CView.text(".  Let Isabella show you how ");
//             if (isabellaFollowerScene.isabellaAccent()) CView.text("es ist");
