...
import { describeCocksLight, describeCock, describeOneOfYourCocks } from 'Content/Descriptors/CockDescriptor';
...
import { telAdreMenu, barTelAdre } from 'Content/Scenes/Places/TelAdre';
import { firstVisitPart1 } from 'Content/Scenes/Places/TelAdre/UmasShop';

export const LoppeFlags = Flags.register("Loppe", {
    LOPPE_FURRY: 0,
    LOPPE_DISABLED: 0,
    LOPPE_MET: 0,
    LOPPE_TIMES_SEXED: 0,
    TIMES_ASKED_LOPPE_ABOUT_LOPPE: 0,
...
// Appearance (edited) (C)
function appearanceOfLoppe(player: Character): NextScreenChoices {
    CView.clear();
    if (LoppeFlags.LOPPE_FURRY === 0) {
        CView.text("Loppe is a 6'2\" bunny-girl with deep brown eyes set in her pretty face.  Shoulder-length black hair and a pair of snow-white rabbit ears adorn the top of her head, tilted and lying backward to conceal the insides.  Curiously, her face is that of a normal human woman.");
    ...
    return { next: loppeGenericMeetings };
}

// First Meeting (edited) (C)
// Happens randomly when choosing to go into the bar after 16:00
export function loppeFirstMeeting(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Wandering towards the bar, you notice an unusual commotion; there are a lot more people hanging around here than is usual for the time of day.  Feeling curious, you go over to investigate, and ask a bystander what all of this is about.");
    ...
    return {
        choices: [
            ["Yes", yesToMeetingLoppe],
            ["Not Now", mebbeLaterToLoppe],
            ["Uninterested", noLoppeInterest],
        ]
    };
...
// [=Uninterested=]
function noLoppeInterest(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Dancers aren't really your thing, and you shake your head.");
    // (disable repeat and NPC)
    LoppeFlags.LOPPE_DISABLED = 1;

    return { next: barTelAdre };
}

// [=Not Now=]
function mebbeLaterToLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You're not really in the mood for this right now, so you leave the bar.");
...
// [=Yes=]
function yesToMeetingLoppe(player: Character): NextScreenChoices {
    CView.clear();
    LoppeFlags.LOPPE_MET = 1;
    CView.text("There's no harm in staying a while...");
    ...
    return { next: yesToLoppeMeetingTwo };
}

function yesToLoppeMeetingTwo(player: Character): NextScreenChoices {
    CView.clear();

    CView.image("loppe-show-at-bar");
    CView.text("Suddenly the bar goes silent and the lights dim; a figure clad in a white mantle steps onto the stage; discarding the covering with a swift movement, it reveals the comely visage of a bunny-girl - a literal bunny-girl, in that apart from the obvious bunny ears and the hints of rabbit-like legs, she actually looks otherwise human despite the prevalence of anthropomorphic animals in this city.");
    ...
    // [(corr < 40)]
    if (player.stats.cor < 40) CView.text(" as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home");
    CView.text(".");
    ...
    return {
        choices: [
            ["Not Really", notReallyInterestedInLoppe],
            ["Sure", sureBakeryWithLoppe],
        ]
    };
}
// [=Not Really=]
function notReallyInterestedInLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decline, and the girl gives you a look of disappointment.");
...
// [=Sure=]
function sureBakeryWithLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You consider the time of day, and the girl offering, and decide that it can't hurt.  Loppe smiles and takes your hand, leading you towards a nearby bakery.");
    ...
    if (player.inventory.gems >= 30) {
        ...
        // [(corr > 40)
        if (player.stats.cor > 40) CView.text("  A small voice inside wonders if she'd eat so extravagantly were she footing her own bill.");
        // remove 30 gems
...
function NoWayLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Loppe lets out a breath she had obviously been holding in a bitter sigh, the ghost of the words, \"<i>I knew it</i>\" echoing faintly as she does so.  \"<i>Well, I understand.  I just had to give it a try all the same.  It was nice talking to you, [name].  Might see you again, someday.  Sorry for wasting your time...</i>\"");
...
function okayLoppeLetsGo(player: Character): NextScreenChoices {
    ...
    return { yes: yesLoppesHouse, no: noLoppesHouse };
}

// [=No=]
function noLoppesHouse(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Loppe gives a disappointed sigh, but smiles at you all the same.  \"<i>I understand... if you ever feel like talking some more, you can find me in the gym.  I need to keep myself in tip-top shape for my dancing shows, after all.</i>\"");
...
// [=Yes=]
function yesLoppesHouse(player: Character): NextScreenChoices {
    CView.clear();
    // Loppe can now be found in the Gym
    CView.text("The dancer smiles mischievously at you.  \"<i>Wonderful.  I'm going to show you just how great my body looks without this dress.  You wouldn't believe how much time I spend in the gym, working out.</i>\"  Loppe grabs her last cookie in one hand and your arm in the other, leading you away - presumably to her house.  Enroute, she giggles constantly, provoking a question from you.");
    ...
    return { next: choiceWrap(loppeSexChoice, true) };
}

// Generic Meeting (edited) (C)
// Loppe can be found from 6:00 to 15:00 (yeah she works out like a boss!)
export function loppeGenericMeetings(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to approach the bunny-girl.  Loppe smiles and wipes the sweat off her brow with the towel.  \"<i>Hey there, [name], nice seeing you around here.  So... do you want to do something?  Talk, maybe?  Or go to my place for a 'workout'?</i>\" she asks with a smirk.");
    // Appearance
    // Talk
    // Sex
    ...
    choices[0] = ["Appearance", appearanceOfLoppe];
    choices[1] = ["Talk", talkWithLoppe];
    choices[2] = ["Sex", choiceWrap(loppeSexChoice)];
...
// Talk (edited) (C)
function talkWithLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("That 'workout' is going to have to be postponed; you have some questions.");
    ...
    const choices: ScreenChoice[] = [];
    // Loppe
    choices[0] = ["Loppe", talkWithLoppeAboutLoppe];
    // Children (available after having sex with Loppe at least once)
    if (LoppeFlags.LOPPE_TIMES_SEXED > 0) choices[1] = ["Children", askLoppeAboutChildren];
    // Gossip (after sexin' once)
    if (LoppeFlags.LOPPE_TIMES_SEXED > 0) choices[2] = ["Gossip", gossipWithLoppe];
    // Working (after [Loppe]
    if (LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE > 0) choices[3] = ["Working", talkWithLoppeAboutWorking];
    // Her Mom (available after choosing \"<i>Loppe</i>\" at least once)
    if (LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE > 0) choices[4] = ["Her Mom", chatWithLoppeAboutHerMom];
    // Her Village (after [Loppe]
    if (LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE > 0) choices[5] = ["Her Village", chatWithLoppeAboutLoppesVillage];
    // The curse (unfinished; available after choosing \"<i>Loppe</i>\" at least once)
    // Fondle (after "Loppe"; see appropriate section)
    if (LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE > 0 && LoppeFlags.LOPPE_TIMES_SEXED > 0) choices[6] = ["Tease Her", choiceWrap(fondleAndTease)];
    choices[9] = ["Back", loppeGenericMeetings];

    return { choices };
}

// Loppe (edited) (C)
function talkWithLoppeAboutLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You tell the laquine that you're curious about her, that you'd like to know more about her: who she is, where she comes from, et cetera.");
    ...
    // (if first dialogue)
    if (LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE === 0) {
    ...
    // set LoppeChat = 1
    LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE = 1;
    return { next: passTime(1) };
}

// Children (edited) (C)
// req LoppeSexed > 0 and LoppeChat > 0
function askLoppeAboutChildren(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide to ask Loppe for her thoughts on children.  Has she considered having any?");
    ...
    return {
        choices: [
            ["Don't Want", loppeIDontReallyWantKidsYouStupidTwat],
            ["Maybe", loppeKidsAreOkayIfYoureARabbitOrSumthin],
            ["Someday", loppeIWantKidsSomedayJeezeQuitHasslingMe],
            ["Soon", loppeIWantKidsSoonOkayCanWeFuck],
            ["No Opinion", noOpinionOnLoppeArt],
        ]
    };
}

// [No Opinion] (spacebar default)
function noOpinionOnLoppeArt(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decline to answer, waving the question off.  Loppe raises an eyebrow at that.  \"<i>Haven't thought about it either, huh?</i>\"");
...
function loppeIDontReallyWantKidsYouStupidTwat(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You confess you aren't really a 'kids' person; you don't think you could ever envision yourself as a parent, especially in this crazy world.");
...
// [=Someday=]
function loppeIWantKidsSomedayJeezeQuitHasslingMe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You always saw yourself as having children at some point in your future.  Coming through the portal, though, you gave up on that dream; when you first arrived, it seemed like the world had nothing in it but imps and goblins and other twisted monsters - what kind of family could you make with people like that?");
...
// [=Soon=]
function loppeIWantKidsSoonOkayCanWeFuck(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a faint smile, you declare that having children is definitely in your future.  You want to be a parent; all you need is to find somebody who feels similar to you.");
...
function gossipWithLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask Loppe if she has any comments on the other residents of Tel'Adre.  The laquine smiles at you.  \"<i>It's not polite to pry into other people's lives, y'know?</i>\"");
    ...
    // Urta
    choices[0] = ["Urta", gossipWithLoppeAboutUrta];
    // Scylla (Must have helped her enough times to know she needs cum to survive.)
    if (LoppeFlags.NUMBER_OF_TIMES_MET_SCYLLA > 0) choices[1] = ["Scylla", gossipWithLoppeAboutScylla];
    // Jasun
    if (LoppeFlags.UNKNOWN_FLAG_NUMBER_00179 > 0) choices[2] = ["Jasun", gossipWithLoppeAboutJasun];
    // Heckel
    if (LoppeFlags.MET_HECKEL > 0) choices[3] = ["Heckel", gossipWithLoppeAboutHeckel];
    // Edryn
    choices[4] = ["Edryn", gossipWithLoppeAboutEdryn];
    // Lottie
    if (LoppeFlags.UNKNOWN_FLAG_NUMBER_00281 > 0) choices[5] = ["Lottie", gossipWithLoppeAboutLottie];
    // Cotton
    if (LoppeFlags.COTTON_MET_FUCKED > 0) choices[6] = ["Cotton", gossipWithLoppeAboutCotton];
    // Back (spacebar default)
    choices[9] = ["Back", talkWithLoppe];

    return { choices };
}

// Urta:
function gossipWithLoppeAboutUrta(player: Character): NextScreenChoices {
    CView.clear();
    // (if UrtaSex or UrtaLover flags are NOT active)
    if (LoppeFlags.TIMES_FUCKED_URTA <= 0 || LoppeFlags.URTA_COMFORTABLE_WITH_OWN_BODY === -1) {
        CView.text("\"<i>Ah, so you've met Watch Captain Urta?  She's famous around here, you know; people say she's a legendary bare-knuckle brawler and one of the toughest guards on the force.  The only problem is that she's not really that easy to approach... I guess she prefers to keep to herself.  Although she acts very friendly with that pretty centauress, Edryn.</i>\"");
    ...
    else if (LoppeFlags.LOPPE_URTA_CHATS === 0) {
        CView.text("Loppe smirks at you.  \"<i>I heard she's been getting along nicely with a certain outsider; you wouldn't happen to know anything about that, would you, [name]?</i>\"");
        // [It's Me] [No]

        return {
            choices: [
                ["It's Me", itsMeFuckingUrtaLoppe],
                ["No", noLoppeWhosFuckingUrta],
            ]
        };
    }
    // (if LoppeUrtaKnowledge > 0)
    else {
        CView.text("\"<i>Urta again, huh?</i>\"  Loppe stares you, but her curious expression turns into one of smug triumph, like a cat that's caught a mouse.  \"<i>So... she has a big horse-cock.  I'm beginning to see a pattern here, sugar.   You like women with huge equine parts?  Is that why you hang out with me?  With Urta?</i>\"");

        // [Play Along][Admit][Deny]
        return {
            choices: [
                ["Play Along", playAlongWivLoppesesUrtaGossip],
                ["Admit", admitToLoppeThatYouLoveZeHorsecock],
                ["Deny", denyToLoppeThatYouLoveZeHorsecock],
            ]
        };
    }
}
...
// [=No=]
function noLoppeWhosFuckingUrta(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You shake your head, claiming that you have no idea what she's talking about.  Loppe looks at you, then shrugs, clearly not caring if you don't have any gossip on the subject to share.");
    // end scene
    return { next: passTime(1) };
}
// [=It's Me=]
function itsMeFuckingUrtaLoppe(player: Character): NextScreenChoices {
    CView.clear();
    // set LoppeUrtaKnowledge = 1
    LoppeFlags.LOPPE_URTA_CHATS = 1;
...
// [=Play Along=]
function playAlongWivLoppesesUrtaGossip(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("And... what would she do if, hypothetically, you <i>did</i> have a fetish for dickgirls ready to give a stallion an inferiority complex?");
    ...
    // Tease Menu options!
    return fondleAndTease(player, false);
}

// [=Admit=]
function admitToLoppeThatYouLoveZeHorsecock(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You look her in the eyes and tell her she's hit the nail on the head; you do have a fetish for chicks with dicks, and having a horsecock makes a dickgirl even sexier to you.");
    ...
    return { yes: choiceWrap(loppeSexChoice, true), no: dontLoppesHouse4Fucks };
}

// [Admit -> No]
function dontLoppesHouse4Fucks(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Loppe raspberries you.  \"<i>You tell me you love herms with big horse cocks and now you won't have sex with me?  You're one big tease, [name].</i>\"");
...
// [=Deny=]
function denyToLoppeThatYouLoveZeHorsecock(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Not in the mood to play along with Loppe's little game, you simply state that your reasons are your own and will stay that way for now.");
...
// Scylla:
function gossipWithLoppeAboutScylla(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>The nun who likes sucking on dicks?</i>\" Loppe asks you.  \"<i>Yes, I've run into her at the Wet Bitch.  She offered to blow me too.  I was tempted, y'know?  She has those wonderful looking lips... nice big breasts too... but ultimately I wound up refusing.  It's just... not right.</i>\"");
...
function gossipWithLoppeAboutHeckel(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask if Loppe's ever had a run-in with a herm hyena who's usually running in the track at the gym.");
...
// Edryn:
function gossipWithLoppeAboutEdryn(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>That pretty centauress watchwoman that hangs in the Wet Bitch when she's off duty?  Yes, it's no secret that she specializes in particular sorts of wetwork for those with... compatible endowments,</i>\" Loppe says, giving you a lecherous wink.");
...
// Lottie:
function gossipWithLoppeAboutLottie(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>That pig-girl that started hanging around the gym in the evenings?  I heard about her... I believe she wants to get fit,</i>\" Loppe comments conversationally.");
    // (if PC's training Lottie)
    if (LoppeFlags.UNKNOWN_FLAG_NUMBER_00299 === 1) {
    ...
    else if (LoppeFlags.UNKNOWN_FLAG_NUMBER_00281 > 0) {
...
// Working (edited)
// req LoppeChat > 0
function talkWithLoppeAboutWorking(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Her curse naturally impacts her social life, but what about work?  How do she and her mother put food on the table?");
    ...
    // set LoppeChat = 2
    LoppeFlags.TIMES_ASKED_LOPPE_ABOUT_LOPPE = 2;
    return { next: passTime(1) };
}

// Her Mother (edited)
function chatWithLoppeAboutHerMom(player: Character): NextScreenChoices {
    CView.clear();
    // req LoppeChat > 0
    CView.text("Loppe rubs her chin, deep in thought.  \"<i>Where do I start?  I guess by saying that my mom is a super mom.  She raised me all on her own, and she's always had time for me, despite having to keep up with her job.  I love her very much.</i>\"");
    ...
    // (If PC is male:)
    if (player.gender === 1) CView.text("  Well, from what Loppe's told you, Uma wouldn't be attracted to you anyway.");
    ...
    if (LoppeFlags.LOPPE_PC_MET_UMA === 0) {
...
// Her Village (edited)
// req LoppeChat > 0
function chatWithLoppeAboutLoppesVillage(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask if Loppe would be willing to tell you about her village.");
...
// Sex
function loppeSexChoice(player: Character, bakery: boolean = false): NextScreenChoices {
    CView.clear();
    // First Time Intro (edited)
    if (LoppeFlags.LOPPE_TIMES_SEXED === 0) {
        // first-time only output, if not coming from bakery date
        if (!bakery) {
            CView.text("That 'workout' seems like a nice option right now.");
            ...
            // (else)
                ...
                // [(lib>40)]
                if (player.stats.lib >= 40) CView.text(".. it almost sounds exciting.");
            }
        ...
        CView.text("It's not very difficult to reach Loppe's place; it happens to be located only a few blocks away from the gym, behind a small shop with the sign \"<i>Kemono's Oriental Clinic</i>\".");
        ...
        // [(corr < 40)]
        if (player.stats.cor < 40) CView.text("  You tell her they're quite nice... though privately you're curious what the interior looks like.");
        else CView.text("  They don't seem to work very well - after all, <i>you're</i> here.");
        ...
        if (player.body.cocks.length > 0) {
            if (player.body.cocks.sort(Cock.Smallest).get(0)!.area <= loppeCapacity()) CView.text("Pulling her onto your dick would result in some cowgirl fun, though you'd have to deal with her cock pointed right at you when she came.  ");
            else CView.text("As good as it would feel to shove your [cock smallest] into her cunt, the nervous looks she's giving it tell you what her response would probably be.  ");
        }
        // [(any cock)
        if (player.body.cocks.length > 0) CView.text("You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ");
        if (player.body.vaginas.length > 0) CView.text("You could take her monster length vaginally, but if her libido is what she claims, you'll probably wind up quite stretched.  ");
        // [(boobs >= boobjob req)
        if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 5) CView.text("Her hardening horsecock looks like it would fit between your [chest], an act likely to net you a messy demonstration of her enthusiasm.  ");
        CView.text("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
        // Display sex options
        // [Cowgirl][Frot][TakeVaginal][Boobjob][TakeAnal][Bail]
    }
    // Repeat Intro (edited)
    else {
        CView.text("Loppe's other 'workout' sounds pretty good right now.");
        ...
        CView.text("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
        // [(loppesexed)
        CView.text("  Just jerking that cock and squeezing it so hard her balls swell up when she comes, though, that might be fun.  It'd be interesting to see how big they can get.");
    ...
    if (player.body.cocks.length > 0 && player.stats.lust >= 33) {
        if (player.body.cocks.find(Cock.CockThatFits(loppeCapacity())))
            choices[0] = ["Cow-girl", loppeRidesCocks];
    }
    if (player.body.cocks.length > 0 && player.stats.lust >= 33)
        choices[1] = ["Get BJ", loppeWorshipsDicks];
    if (player.body.vaginas.length > 0 && player.stats.lust >= 33)
        choices[2] = ["TakeVaginal", getFuckedInYerTwatYaCunt];
    if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 4) choices[3] = ["Boob-job", boobjobLoppe];
    if (LoppeFlags.LOPPE_TIMES_SEXED > 0) choices[4] = ["SqueezeJob", loppeSqueezedickWhateverThatIs];
    if (player.body.legs.isTaur() && player.stats.lust >= 33) choices[5] = ["TakeAnal", getAssFuckedByLoppeAsACentaur];
    else if (player.stats.lust >= 33) choices[5] = ["TakeAnal", getButtFuckedNonHoarseByLoppe];
    choices[9] = ["Leave", beATeaseAndLeaveLoppeAfterSexInvite];
...
function loppeRidesCocks(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-rides-your-cocks");
    CView.text("Looking over the hermaphroditic, horse-cocked bunny-girl, you contemplate your options.  You settle yourself ");
    // [(not centaur)]
    if (!player.body.legs.isTaur()) CView.text("on her bed, making yourself comfortable, and start suggestively stroking yourself");
    // (centaur)
    else CView.text("cumbersomely on the floor, rolling over and spreading your hindlegs to expose yourself");
    CView.text(".  Quietly, you ask Loppe how she would feel about indulging her feminine half.");

    CView.text("\n\n\"<i>I would love to, sugar!</i>\"  Loppe gazes at your " + describeCocksLight(player) + ".");
    // [(2 fit cocks)
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(loppeCapacity()));
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(loppeCapacity())).get(1);

    if (secondCockThatFits && player.body.cocks.length === 2) CView.text("  \"<i>In fact... I could just eat both of those up.</i>\"");
    // (3+ fit cocks)
    else if (secondCockThatFits && player.body.cocks.length === 3) CView.text("  \"<i>But I'm afraid I'll only be able to handle two of them.</i>\"");
    if (player.body.cocks.length > 1 && secondCockThatFits) CView.text("\n\nBetter than the usual reaction... at least she's willing to give more than one some love.");

    CView.text("\n\nLoppe pushes you on your back and gently takes hold of ");
    if (secondCockThatFits) CView.text("two of ");
    CView.text("your cock");
    if (secondCockThatFits) CView.text("s");
    CView.text(", rubbing gently.  \"<i>Just sit back and leave everything in my capable hands, sugar.</i>\"");
    ...
    if (secondCockThatFits) CView.text("s");
    CView.text(" nice and slick.  You gasp in pleasure and surprise as you feel Loppe's erect cock sidle up to yours, helping lube you with its leaking pre. Raising up your hips, you clumsily try to ");
    if (!secondCockThatFits) CView.text("slide your shaft against her");
    else CView.text("pinion her shaft between your");
    CView.text(" own, shivering from the sensation of your sensitive " + describeSkin(player) + " against her proud horseflesh, already drooling even though you can plainly feel that it's only half-erect.");
    if (player.body.balls.count > 0) CView.text("  Your [balls] gently brush and rub against her own swollen cum-factories, and you can't wait to empty your overfilled sac into her waiting womb.");

    CView.text("\n\n\"<i>Okay, that's enough foreplay!</i>\" Loppe announces suddenly, eyeing your " + describeCock(player, cockThatFits) + " with a hunger that you never expected to see on her face.  Loppe quickly straddles you, aligning it with her pussy");
    if (secondCockThatFits) CView.text("; its neighbor is aimed for her tight rosebud");
    CView.text(".  \"<i>Itadakimasu!</i>\" Loppe says, licking her lips as she finally slides you home.");
    ...
    CView.text(", yet squeezing you as tightly as a velvety vise.  In fact, she's gripping you so hard that you can't seem to push yourself any further into her depths; she's squeezing you much too tightly to move in or out, an outright amazing feat, if Loppe gets half as much action as she claims to.");
    ...
    if (secondCockThatFits) CView.text("s");
    CView.text(" with happy squeals of pleasure and happiness, softly accompanied by the wet sounds of your coupling.  You gasp and cry, savoring every motion as she squeezes and wriggles, her inner walls rippling so deliciously around your cock");
    if (secondCockThatFits) CView.text("s");
    CView.text(", stroking them in the most delicious way.  You thrust your hips, trying to slip deeper inside her, but still she's holding you at bay.");
    ...
    if (secondCockThatFits) CView.text(" and watertight ass");
    CView.text(".  The sensation borders on the thin line between pleasure and pain, and you can't help but moan into Loppe's kiss; you wrap your ");
    if (player.body.legs.isDrider()) CView.text("spindly legs");
    else if (player.body.legs.isTaur()) CView.text("forelegs");
    else CView.text("arms");
    CView.text(" around the bunny-girl's ");
    if (LoppeFlags.LOPPE_FURRY === 0) CView.text("silky-smooth");
    else CView.text("softly furred");
    CView.text(" back and hold her tight against you, her long horse-cock sandwiched between your bellies, smearing both of you with drooling precum, rivers of the stuff flowing out steadily with every thrust and grind the two of you make.");
    ...
    if (!player.body.legs.isTaur()) {
        CView.text("<b>You realize that if you keep holding onto Loppe, you're going to end up with a face covered in herm-cum - if you act quickly, though, you can avoid the impromptu facial.  You could even turn her hose of a cock back on her.</b>");
        // [NoFace] [Facial] [HoseHer]

        return {
            choices: [
                ["NoFace", loppeRidesYouNoFaceJizz],
                ["Facial", loppeRidesYouSpunksInYourEye],
                ["HoseHer", loppeRidesYouHoseHer],
            ]
        };
    }
    else {
        return { next: loppeRidesYouNoFaceJizz };
    }
}

// {If NoFace:
function loppeRidesYouNoFaceJizz(player: Character): NextScreenChoices {
    CView.clear();
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(loppeCapacity())).get(1);

    CView.text("You let go of the horse-dicked rabbit and she gratefully swivels herself fully upright, cock jutting out over your belly - instinctively, your ");
    if (player.body.legs.isTaur()) CView.text("foreleg tucks under her cock, angling it away from your body.");
    ...
    return loppeRidesPCCockFinal(player);
}

// {If Facial:}
function loppeRidesYouSpunksInYourEye(player: Character): NextScreenChoices {
    CView.clear();
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(loppeCapacity()));
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(loppeCapacity())).get(1);

    CView.text("You keep your ");
    if (player.body.legs.isDrider()) CView.text("legs");
    else CView.text("arms");
    CView.text(" wrapped around your lover, welcoming what's approaching as her horse-prick throbs and her flared tip inflates.  With a rapturous howl the laquine's cumslit opens like a floodgate, spurting jet after jet of cum onto your [chest], [face] and even the wall behind you; her own pillowy breasts and face are splashed with the force of her orgasm.  Her tight pussy contracts, milking your " + describeCock(player, cockThatFits) + " for all its worth");
    if (secondCockThatFits) CView.text(", while her anal ring constricts your " + describeCock(player, secondCockThatFits) + " tightly, intent on holding you in place");
    CView.text(".");
...
// {If HoseHer:
function loppeRidesYouHoseHer(player: Character): NextScreenChoices {
    CView.clear();

    CView.text("You let go of Loppe, and she gratefully swivels herself fully upright, cock jutting out over your belly - your hand lunges forward and places itself palm upright underneath the shaft, fingers curling around as you push it further up and up until it's pointing, as best you can make it, at her face!  You can feel it throbbing like mad against your fingers, can feel the impressive bulge of semen as it distends her urethra and surges up towards her flared tip.  With an ululation of ecstasy, she literally explodes into orgasm, cum geysering from her cock and squarely into her face, causing her to choke and splutter at the surprise facial, the pearly spooge splattering down her throat and onto her breasts.  With a laugh she shakes her head, closing her eyes and opening her mouth; even as she continues to bounce and buck and grind against you, her cock keeps spurting cum, and she clumsily tries to catch it, eagerly swallowing down mouthful after mouthful.  She's actually quite good at it, but despite her best efforts, her hair, ears, face and breasts end up completely plastered in cum... not that this stops her from milking you with her lower hole");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(" until you can't help it and cum yourself.");
    return loppeRidesPCCockFinal(player);
}

function loppeRidesPCCockFinal(player: Character): NextScreenChoices {
    const cockThatFits = player.body.cocks.find(Cock.CockThatFits(loppeCapacity()));
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(loppeCapacity())).get(1);

    CView.image("loppe-rides-your-cock");
    CView.text("\n\nYour " + describeCock(player, cockThatFits) + " gushes fluids into her hungry womb");
    if (secondCockThatFits || player.body.vaginas.length > 0) {
        CView.text(", while ");
        if (secondCockThatFits) CView.text("your second " + describeCock(player, secondCockThatFits) + " just as easily pumps her perverted ass full of baby juice");
        if (secondCockThatFits && player.body.vaginas.length > 0) CView.text(" and ");
        if (player.body.vaginas.length > 0) {
            CView.text("your [vagina] ");
            if (player.body.vaginas.get(0)!.wetness < 3) CView.text("drizzles");
            else if (player.body.vaginas.get(0)!.wetness < 5) CView.text("spatters");
            else CView.text("gushes");
            CView.text(" femcum onto the bed below you, forgotten during the act but not unsympathetic to her ministrations");
        }
    }
    CView.text(".  You cry out as you cum long and hard, arching your back off of the bed in your pleasure before finishing and collapsing, gasping for breath as, above you, the laquine comes to her own climactic end.");
    ...
    if (player.body.legs.isDrider() || player.body.legs.isTaur()) CView.text("forelegs");
    else CView.text("arms");
    CView.text(" around her, pulling her into a post-coital cuddle.  Her wet cock quietly slaps against your belly as she embraces you back, clearly savoring the closeness the same way you are.  However, moments later, you feel Loppe's hips start to twitch, slowly building into more lustful thrustings, seconds before Loppe pushes herself back into a deliberately upright position, her pace building steadily.  With a lusty smile and a mischievous twinkle in her eyes, Loppe looks you straight in the eye as she proclaims, \"<i>It's time for round two, [name]!</i>\"");
    ...
    CView.text(describeCocksLight(player) + " and slides down your body, pressing her soft, cum-slickened breasts against you.  You moan as the smooth curves of her chest press against [eachCock]; the cum-slick ");
    if (LoppeFlags.LOPPE_FURRY === 0) CView.text("skin");
    else CView.text("fur");
    CView.text(" give you the impression of being wrapped in the moist folds of a woman's sex.  As Loppe slides further down you feel her teasingly licking at the tip");
    if (secondCockThatFits) CView.text("s");
    CView.text(" of your " + describeCocksLight(player) + ".");

    CView.text("\n\nThe skillful dance of her warm, wet tongue on the sensitive skin of your post-orgasmic member");
    if (secondCockThatFits) CView.text("s");
    CView.text(" pull");
    if (!secondCockThatFits) CView.text("s");
    CView.text(" an involuntary shudder from you; the erotic stimulation sends blood pulsing back into your veins, and your shaft");
    if (secondCockThatFits) CView.text("s rise");
    else CView.text(" rises");
    CView.text(" to full mast again, so swiftly ");
    if (!secondCockThatFits) CView.text("it almost smacks");
    else CView.text("they almost smack");
    CView.text(" the laquine on her nose, leaving you ready to be mounted by her once again.");
    ...
    if (player.stats.minLust() <= 30 && player.stats.lib <= 50) CView.text("; if I have to stop to tend to you after every coupling, I'll never get these empty");
    CView.text(".</i>\"  She points with a smirk to her swollen balls.  Er... didn't she just blow her load? Her balls look at least as full as, if not fuller than, when you started, and they nearly slosh with eagerness to be rid of their cargo!");
    ...
    // [(Exhibitionist)
    if (LoppeFlags.PC_FETISH > 0) CView.text("; it gives you something of a thrill, though nowhere near as much as taking them off for her in the first place did");
    CView.text(".  Finished, you depart.");
    player.orgasm();
    player.stats.tou += .5;
    player.stats.lib += .5;
    player.stats.sens += -4;

    LoppeFlags.LOPPE_TIMES_SEXED++;
    // 3 hours pass.
...
function loppeWorshipsDicks(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-worships-your-cock");
    CView.text("As your eyes sweep over the naked half-breed's form, they focus on her equine masculinity, already standing proud and eager at the prospect of sex.  Distinctly aware of your own male appendage");
    if (player.body.cocks.length > 1) CView.text("s");
    CView.text(", you conversationally ask Loppe what she thinks of your [cock biggest], an idea starting to form in your mind.");

    // (if cock area <= 17.5)
    if (player.body.cocks.sort(Cock.Largest).get(0)!.area <= 17.5) {
    ...
    else if (player.body.cocks.sort(Cock.Largest).get(0)!.area <= 35) {
        ...
        if (player.body.cocks.sort(Cock.Largest).get(0)!.area > loppeCapacity()) CView.text("even if it does look too big to fit.");
        else CView.text("or maybe I should get a taste first?");
        CView.text("</i>\"  She licks her lips.");
    }
    ...
    if (player.body.balls.count > 0) CView.text(" and balls to balls");
    CView.text(", the delicious friction pushing you over the edge.");
...
function getFuckedInYerTwatYaCunt(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-sticks-it-in-your-vagoo");
    CView.text("As you contemplate the delicious possibilities that the herm presents, your eyes are drawn to her admirable piece of horse-meat.  With a half-grin, you settle yourself on her bed, ");
    // [(mans)
    if (!player.body.legs.isTaur()) CView.text("spreading your labia in an invitation that needs no words.");
    else CView.text("exposing your rump and spreading your hind legs in your best effort to make her feel welcome.");

    CView.text("\n\nLoppe smirks at you and winks.  \"<i>Don't worry; I'll treat you like a ");
    // [(femininity <= 40)
    if (player.body.femininity > 50) CView.text("princess.  A sexy, smoking hot princess");
    else CView.text("prince.  A dashing, dandy prince");
    ...
    // [(herm non-horse)
    if (player.gender === 3 && !player.body.legs.isTaur()) {
        CView.text(", though she needs to move aside " + describeOneOfYourCocks(player) + " ");
        if (player.body.balls.count > 0) CView.text("and [balls] ");
        CView.text("to really get at your cunt");
    }
    CView.text(".");
    ...
    // [(not horse and herm)
    if (!player.body.legs.isTaur() && player.gender === 3) {
        CView.text(", under your ");
        if (player.body.balls.count > 0) CView.text("[sack]");
        else CView.text(describeCocksLight(player));
        CView.text(",");
    }
    CView.text(" to lick and kiss your sensitive labia.  You arch your back and thrust your crotch at her, letting the laquine have your precious pussy to taste and suckle and ravage... she's pretty good with that tongue of hers.  Loppe lifts her head and wipes her face with her arm.  \"<i>I'm even better with this,</i>\" she says, mirroring your thoughts and holding her painfully erect equine-shaft.  You watch as it throbs, every vein bulging visibly, balls engorged as they churn up a load of cum to deposit in your waiting womb.  She ducks to take another lick, tasting you like one tastes a fine wine.");
    ...
    if (player.body.vaginas.get(0)!.virgin) {
        ...
        // [(low corr)]
        if (player.stats.cor < 50) CView.text("  You look coy, for her benefit, and cast your eyes down shyly.");
        else CView.text("  You do your best to warn her away from the topic with your eyes, eager to get on with the fuck.");
        // (not for horsez)
        ...
        CView.text("\n\nYou reach out ");
        if (player.stats.cor < 50) CView.text("and tap her playfully on the nose");
        else CView.text("in your irritation and squeeze her face imploringly");
        CView.text(".  Would you be here, with your clothes off in Loppe's bed, if you weren't okay with it?");
    ...
    if (player.vaginalCapacity() >= 35) CView.text(" bottoms out, completely hilted inside you");
    // [else:
    else CView.text(" bumps against your cervix");
    CView.text(". You moan in delight as she sinks into you");
    if (player.body.vaginas.get(0)!.virgin) CView.text(", repressing a shiver of pain as she tears through your hymen, taking your virginity");
    else if (player.vaginalCapacity() < 35) CView.text(", forcing your entrance wide to accommodate herself");
    CView.text(".");
    // cuntchange, but suppress standard messages
    displayStretchVagina(player, 35, true, true, false);
    ...
    // [(not horse)
    if (!player.body.legs.isTaur()) CView.text("  You wrap your [legs] around her hips as best you can, thrusting in your efforts to meet her strong rhythm, feeling your cunt rippling around her wonderfully long, thick horsecock.");
    ...
    if (player.body.cocks.length > 0) CView.text(", precum begins to drool from your " + describeCocksLight(player));
    CView.text(".");
    ...
    if (!player.body.legs.isTaur()) {
        ...
        if (player.gender >= 3) CView.text(" and smearing both your bellies with herm-pre");
        CView.text(".");
    }
    // Centaur:
    else {
    ...
    if (player.body.cocks.length > 0) {
        CView.text(", neglected cock fountaining cum ");
        if (player.cumQ() < 500) CView.text("between you and painting you both");
        else CView.text("gushing onto the bedsheets and smearing your belly");
        CView.text(" with this aspect of your dual-sexed orgasm");
    }
    CView.text(".");
    ...
    if (!player.body.legs.isTaur() && player.body.cocks.length > 0) CView.text(", giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin");
    CView.text(".");
    ...
    // [(low corr)
    if (player.stats.cor < 50) CView.text("a pleading look... and if there's maybe a trace of horror dancing in the depths of your eyes, well, that's understandable, isn't it?");
    else CView.text("an incendiary glare, letting her know just what you think of her posturing.");
    CView.text("  Loppe bursts out laughing.  \"<i>Just kidding, sugar.  If we go another round I think my dick might fall off.</i>\"");
    ...
    if (Settings.sillyMode) CView.text("  \"<i>Da! Sandvich is kredit to team!</i>\"");
    ...
    // [Libido >=50:
    if (player.stats.lib >= 50 || player.stats.minLust() >= 30) CView.text("  Not that you didn't just titillate yourself thinking of it...");
    CView.text("  Your lover laughs, \"<i>Okay, you got me there... but, anyway, I have some chores to run for my mom, so I guess I'll see you later?</i>\"");
...
function getButtFuckedNonHoarseByLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-sticks-it-in-your-butt");
    CView.text("You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl's legs, and know you just have to have it.  Slowly, you spread yourself upon Loppe's bed, scooting up toward the head and pillows and kneeling, [butt] in the air, inviting the laquine to do with you as she will.");
    ...
    if (player.body.butt.looseness === 0) {
        ...
        displayStretchButt(player, 35, true, true, false);
    ...
    else if (player.body.butt.looseness < 3) {
    ...
    if (player.body.cocks.length > 0) {
        CView.text(", cock");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" dripping pre");
        if (player.body.vaginas.length > 0) CView.text(" and");
    }
    else if (player.body.vaginas.length > 0) CView.text(", ");
    if (player.body.vaginas.length > 0) CView.text(" pussy oozing femlube");
    CView.text("... you beg her to go faster, to do it harder!");

    CView.text("\n\nLoppe happily complies, thrusting herself deeper and stronger.  ");
    // [(cock)
    if (player.body.cocks.length > 0) CView.text("One of her hands reaches around to grasp your pre-slickned [cock biggest], stroking you in time with her thrusts.  ");
    if (player.gender === 2) CView.text("One of her hands reaches around to tease your dripping [vagina] and erect [clit], dipping a finger in every time she bottoms out.  ");
    CView.text("Wet slapping noises fill the room, and you gasp in pleasure each time her cum-churning balls slap against ");
    if (player.body.balls.count > 0) CView.text("yours");
    else CView.text("your [butt]");
    CView.text(".  Loppe lays down on top of you, panting hotly on your neck, moaning in pleasure each time your sphincter contracts.");
    ...
    if (player.body.vaginas.length > 0) {
        CView.text("  A gush of sexual fluids from your crotch ");
        if (player.body.vaginas.get(0)!.wetness < 3) CView.text("spatters");
        else if (player.body.vaginas.get(0)!.wetness < 4) CView.text("splashes");
        else if (player.body.vaginas.get(0)!.wetness < 5) CView.text("drenches");
        else CView.text("completely soaks");
        CView.text(" the bed-linen under you.");
    }
    ...
    if (LoppeFlags.SATYR_KIDS > 0) CView.text("You laugh softly; hopefully she'll never have to meet the creature that dispelled that same doubt in you.  ");
    CView.text("\"<i>I think this is time for sleeping, not joking.</i>\"  With a trace of wounded dignity, she lays herself down and curls into your back, one arm wrapped around your gurgling belly.");
    ...
    // (First Time)
    if (LoppeFlags.LOPPE_TIMES_SEXED === 0) {
        CView.text("Loppe laughs nervously.  \"<i>Sorry about that.  It's just that I usually can't bag a real cutie like you, and I think I might have gone a bit overboard... is there anyway I can make it up to you?</i>\"");
...
function getAssFuckedByLoppeAsACentaur(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-sticks-it-in-your-butt-you-centaur");
    CView.text("You swallow as your eyes are drawn to the equine sausage jutting out between the bunny-girl's legs, and know you just have to have it.  But is she confident in her ability, even with your shape?");
    ...
    // [ass wet)
    if (player.body.butt.wetness > 0) CView.text("  She doesn't even have to bother with lube; your ass is wet enough to accommodate her on its own.");
    ...
    if (player.body.balls.count > 0) CView.text("rs");
    else if (player.body.vaginas.length > 0) CView.text("r pussy");
    CView.text(".");

    CView.text("\n\nYou moan as she gets into her task");
    if (player.body.cocks.length > 0) CView.text(", feeling your erect [cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe's room.  You hope she doesn't mind a few stains...  ");
    // (else vag)
    else if (player.body.vaginas.length > 0) CView.text("; your cunt is sopping, making a small pool of juices one the floor.  ");
    else CView.text(".  ");
    CView.text("She seems more worked up than usual, perhaps because you have a horse-like lower body?  \"<i>Are your instincts acting up?</i>\" you tease her.  \"<i>Or do you just love ass that much?</i>\"");
    ...
    if (player.gender <= 1) CView.text("  Even if you are the wrong sex entirely for that.");
    ...
    // [PC has cock:
    if (player.body.cocks.length > 0) CView.text("  The bumps against your prostate send waves of pleasure running through [eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ");
    CView.text("If she keeps this up for too much longer, you will pop any moment now...");
    ...
    if (player.body.cocks.length > 0) CView.text("  [EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.");
    if (player.body.vaginas.length > 0) CView.text("  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.");
    ...
    if (player.body.skin.type !== SkinType.SCALES) CView.text(" have goosebumps all over your body");
    else CView.text("r scales are practically standing on end");
    CView.text(".");

    CView.text("\n\n\"<i>Ohhh... you are incredible, [name].  ");
    // [PC has dick that fits)
    if (player.body.cocks.find(Cock.CockThatFits(loppeCapacity()))) CView.text("Though I hope you won't mind riding me, sometime,</i>\" she laughs.  \"<i>");
    CView.text("How about another round, hmm?  Big strong 'taur like you should certainly be able to hold a little more loving inside of you, right?</i>\"  She croons happily, running her hands up and down your ass as if trying to entice you.");
    ...
    CView.text("  Anyway, your clothes are back in my room; sorry to fuck and run, but I've got some errands to run before mom gets home - you'll have to see yourself out, okay?</i>\"");
...
function loppeSqueezedickWhateverThatIs(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-orgazm-denial");
    // (if LoppeDenial == 0)
    if (LoppeFlags.LOPPE_DENIAL_COUNTER === 0) {
        CView.text("After seeing Loppe's prodigious output, you wonder how she would handle not being able to orgasm.");
    ...
    else if (LoppeFlags.LOPPE_DENIAL_COUNTER === 1) {
    ...
    if (player.stats.cor < 50) {
        CView.text("  You loosen your grip; this was supposed to be pleasurable, not painful, so it's definitely time to stop.");
    }
    // (High Corruption, no new pg)
    else {
        CView.text("  You suppose you could let her go... on the other hand, it might be fun to see just how big she'll get if you keep holding her in.");
    }

    // Goto [Let Go]
    return {
        choices: [
            ["Let Go", letsLoppeGoCum],
            ["Hold", superLoppeOrgasmDenialGo],
        ]
    };
}

// [=Let Go=]
function letsLoppeGoCum(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-comes-after-orgasm-denial");
    CView.text("As your fingers slacken on the laquine's straining shaft, they're practically pushed away by the violent force of long-delayed ejaculation, which rockets out of her horse-cock with immense speed and volume as you back away.  For an instant you almost think like she's going to punch a hole through the wall.  She screams, both in pleasure and relief as she fountains cum like a perverted geyser.  Spooge flies through the air, spilling all over the room; you manage to avoid the brunt of it, but some droplets inevitably hit you.");
    ...
    LoppeFlags.LOPPE_DENIAL_COUNTER = 3;
    player.stats.raw.lust += 30 + player.stats.lib / 10;

    LoppeFlags.LOPPE_TIMES_SEXED++;
    return { next: passTime(1) };
}
// [=Hold=]
function superLoppeOrgasmDenialGo(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-comes-after-moar-orgasm-denial");
    CView.text("For fear of not being able to hold her back anymore, you return both hands to tightly gripping her shaft.  Tears escape Loppe's eyes as she's denied yet again.  \"<i>Stop...</i>\" she says meekly.");
    ...
    CView.text("\n\n\"<i>Bye, sugar!");
    if (player.body.cocks.length > 0) CView.text("  Maybe next time I'll be the one to give you a bit of a holdout!");
    CView.text("</i>\"");
    // leave, set LoppeDenial flag = 7
    LoppeFlags.LOPPE_DENIAL_COUNTER = 5;
    // (very high corr)
    if (player.stats.cor >= 90) {
        ...
        LoppeFlags.LOPPE_DENIAL_COUNTER = 10;
...
// Needs DD-(cup) boobs.
function boobjobLoppe(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-give-her-a-titty-fuck");
    CView.text("You contemplate your options, unthinkingly rubbing your breasts.  As you do so, you notice your lover is observing your hands very keenly.  With a grin, you grope your tits and ask if Loppe likes your display.");
    ...
    CView.text("\n\nEmboldened, you clasp your breasts firmer together and start to rise and fall, slowly scraping your " + describeSkin(player) + " up and down the half-breed's prick.  It feels so hot and firm against your chest, growing increasingly damp as Loppe oozes gouts as voluminous as the cumshots of ordinary men, making your breasts and her prick alike equally slippery and enabling you to stroke her faster and faster. \"<i>K-keep that up and I'm gonna shoot anytime,</i>\" Loppe groans, trying to hump into your cleavage.");

    // [(Normal/Naga tongue)
    if (player.body.tongue.type < TongueType.DEMONIC) CView.text("\n\nYou bend your head and lean down, letting your tongue flick out of your lips and slide gently across the flared head of Loppe's horse-prick, seductively tasting the sweet-salty flavor of her pre.  \"<i>Ah!  Hmm... I seriously hope you like the taste, because you're getting a faceful anytime now!</i>\"  Loppe moans.");
    // Demon/Dragon:
    ...
    if ((player.stats.str < 50 && !player.body.legs.isTaur()) || (player.stats.str < 30 && player.body.legs.isTaur())) {
        CView.text("  Struggle as you may, you're powerless to stop the drooling laquine as she sends you crashing down on her bed.  The whole piece of furniture shakes ominously with the force of your impact, and for a moment you think it will break apart.");
        ...
        if (player.body.tongue.type > TongueType.HUMAN) CView.text("looping your inhuman tongue around it and stroking for extra stimulus even while ");
        CView.text("grabbing your breasts and squeezing the horsemeat between them.  The sooner you help her get over this frenzy, the sooner she'll get off of you... hopefully.  \"<i>Ah, sugar!  I knew you liked it rough!</i>\" your suddenly forceful lover shouts, ominously.");
        ...
        if (Settings.sillyMode) CView.text(".  You can't help but think of Family Matters");
        CView.text(".  You simply nod and groan, hands cradling your distended gut.  Loppe snuggles up against you, rubbing your bloated midriff.  \"<i>Sorry about that... hmm, how about we just cuddle and rest?</i>\"");
        ...
        CView.text("  You hold the laquine's arms and force her onto the floor; she lands with an audible \"<i>oomph</i>\" and you grin as you hold the eager herm down.");
        ...
        // end scene
        CView.text("\n\n");
        if (player.stats.cor >= 60) CView.text("You don't really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ");
        CView.text("Oh, well... you reach down and heft the surprised hybrid off of the floor and onto her spattered bed, then slide yourself into it as best you can, wrapping your arms around her.  She hugs you back, then yawning widely.  \"<i>Naptime...</i>\"");
    ...
    if ((player.stats.str < 50 && !player.body.legs.isTaur()) || (player.stats.str < 30 && player.body.legs.isTaur())) CView.text("- while your stomach is still heavily swollen and gurgles in protest, it's not bad enough to impair you - ");
    CView.text("before going over and dressing yourself.  Loppe waves goodbye to you as you head out once again.");
...
// Leave (edited)
function beATeaseAndLeaveLoppeAfterSexInvite(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("loppe-teaze-and-leave");
    CView.text("Despite the hybrid beauty standing stark naked before you, you decide that you aren't in the mood after all.");
...
// req LoppeChat > 1
function fondleAndTease(player: Character, output: boolean = true): NextScreenChoices {
    if (output) {
        CView.clear();
        CView.text("You lean in over the table and tell Loppe you'd like to talk about her.  She looks vaguely confused.  \"<i>Well, I already told you about everything I can think of.  Was there something you wanted to explore in more detail?</i>\"");
...
// Suck (edited)(C)
function teaseLoppeNSuck(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You lean over the table and give the laquine a kiss.  Confused, but not displeased, Loppe eagerly returns it, then watches as you break the lip lock and start to slide yourself under the table.  \"<i>Umm... sugar?  What are you doing?</i>\"");
    ...
    if (player.body.tongue.type > TongueType.HUMAN) CView.text(", inhumanly long tongue looping out to coil wetly around the length");
    CView.text(".  You suckle gently, letting the salty tang of hot spunk wash over your tastebuds, and then start to gulp and swallow your way down her prick's sizable length.");
