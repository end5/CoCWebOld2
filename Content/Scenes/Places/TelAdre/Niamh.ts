...
import { isHolidays, isValentine } from 'Content/Utilities/Dates';
import { mfn, mf } from 'Content/Descriptors/GenderDescriptor';
...
import { growTopBreastRowDownwards } from 'Content/Modifiers/BreastModifier';
...
export const NiamhFlags = Flags.register("Niamh", {
    NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER: 0,
    NIAMH_MOVED_OUT_COUNTER: 0,
    MET_NIAMH: 0,
    GOT_NIAMH_BEER: 0,
    TALKED_NIAMH: 0,
    NIAMH_STATUS: 0,
    TIMES_NIAMH_BAZAAR_MET: 0,
    URTA_COMFORTABLE_WITH_OWN_BODY: 0,
});

TimeEvents.register("Niamh", (player: Character): void | boolean => {
    let needNext: boolean = false;
    if (NiamhFlags.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER > 1) NiamhFlags.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER--;
    if (NiamhFlags.NIAMH_MOVED_OUT_COUNTER > 1) NiamhFlags.NIAMH_MOVED_OUT_COUNTER--;
    const bimboChampagneEffect = player.effects.getByName(EffectType.BimboChampagne);
    if (bimboChampagneEffect) {
        bimboChampagneEffect.values.value1 += -1;
        if (bimboChampagneEffect.values.value1 <= 0) {
            removeBimboChampagne(player);
            needNext = true;
        }
    }
    const blackCatBeerEffect = player.effects.getByName(EffectType.BlackCatBeer);
    if (blackCatBeerEffect && blackCatBeerEffect.values.value1 > 0) {
        blackCatBeerEffect.values.value1 += -1;
        if (blackCatBeerEffect.values.value1 <= 0) {
...
export function telAdreNiamh(player: Character) {
    if (NiamhFlags.MET_NIAMH === 0) {
        ...
        if (Time.hour <= 8) CView.text("massive N-cup breasts, spilling over the table");
        else if (Time.hour <= 9) CView.text("huge M-cup breasts, taking up much of the table");
        else if (Time.hour <= 10) CView.text("great L-cup breasts, advancing far onto the table");
        else if (Time.hour <= 11) CView.text("heavy K-cup breasts, lying on the table for support");
        else if (Time.hour <= 12) CView.text("hefty J-cup breasts, resting comfortably on the table");
        else if (Time.hour <= 13) CView.text("weighty I-cup breasts, dangling to lie on the table");
        else if (Time.hour <= 14) CView.text("squeezable H-cup breasts, settling burdensomely on her chest");
        else CView.text("moderately manageable G-cups, trembling with every breath she takes");
        CView.text(".  Smooth, touchable skin covers her chest, face, hands and presumably the rest of her; it seems her animalistic accoutrements stop at kitty ears and tail. She takes a deep swig of her drink, following it up with a cute little burp.");
    }
    // This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
        ...
        if (Time.hour <= 8) CView.text("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
        else if (Time.hour <= 14) {
            CView.text("her ");
            if (Time.hour <= 9) CView.text("M-cup");
            else if (Time.hour <= 10) CView.text("L-cup");
            else if (Time.hour <= 11) CView.text("K-cup");
            else if (Time.hour <= 12) CView.text("J-cup");
            else if (Time.hour <= 13) CView.text("I-cup");
            else CView.text("H-cup");
            CView.text(" brew-burdened breasts have clearly gotten smaller as the day dragged on, although they're still uncomfortably sizeable and swollen.");
        }
...
export function approachNiamh(player: Character): NextScreenChoices {
    ...
    // [Talk] [Get Beer] [Leave]
    return { choices: [["Talk", talkToNiamh], ["Get Beer", beer], ["", undefined], ["", undefined], ["Leave", leaveNiamh]] };
}

// [Get Beer]
export function getANiamhBeer(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("niamh-get-beer");
    CView.text("\"<i>That'll be two gems,</i>\" she replies.  You hand over the payment, which she holds up to the light and scrutinizes thoroughly.  Satisfied, she drops the things into a sack at her waist.  \"<i>Sorry,</i>\" she mutters. \"<i>I do want to get this damned beer out o' me, but this is still my primary livelihood; I canno' afford to be cheated.  Anyway, here's your mug - you can keep that f'r another two gems; I get 'em cheap.  Go ahead and fill it up, but keep in mind what you're tweakin', right?</i>\"");
    // Feeder
    if (player.effects.has(EffectType.Feeder)) CView.text("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
    // (Player has G-cups or bigger;
    else if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 15) {
        if (player.lactationQ() === 0)
            CView.text("  Not terribly surprising; you know your tits are sensitive, and you don't even make milk in them.");
        else
            CView.text("  You can easily imagine just how much trouble she must have, especially given the bulk of her tits are fluid.");
    }
    else if (player.lactationQ() > 0)
        CView.text("  You know just how distracting it is to have your tits full of fluid waiting to get out.");

    // ([if first time]
    if (NiamhFlags.GOT_NIAMH_BEER === 0)
    ...
    if (NiamhFlags.MET_NIAMH > 0)
        ...
        CView.text("the cat-girl ");
    CView.text("sighs in relief, biting her lower lip slightly.");
    // General Ending
    ...
    NiamhFlags.GOT_NIAMH_BEER++;
    return { choices: [["Here", drinkNiamhsBeerInTelAdre], ["To Go", togo]] };
}

// [Here]
function drinkNiamhsBeerInTelAdre(player: Character): NextScreenChoices {
    CView.clear();
    if (player.lactationQ() >= 300 && player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 5 && randInt(2) === 0 && NiamhFlags.MET_NIAMH > 0) {
        ...
        if (isHolidays() && date.date >= 31) {
            ...
            if (player.body.vaginas.length > 0) CView.text("pussy to wetten");
            if (player.gender === 3) CView.text(" and ");
            if (player.body.cocks.length > 0) CView.text("[eachCock] to stiffen");
            CView.text(".  \"<i>Ye know, I got's an idea.  How's about ye stick by me for the next hour.  I'm sure ye must be diein' to drain those big'uns of yours.  We might as well sell your champagne ta the patrons.  Whaddaya say Lassie?</i>\"");
        ...
        return { choices: [["SellYourBooze", sellYourBooze], ["", undefined], ["", undefined], ["", undefined], ["Leave", leaveWithBeerTits]] };
    }
    CView.text("You yell \"<i>Skoal!</i>\" and upend the mug into your mouth, drinking greedily.  The warmth of the booze quickly follows the liquid itself - potent stuff, this is.");
    // PC is affected by Black Cat Beer item effects
    blackCatBeerEffects(player, false, true);
    // both output
    if (Time.hour <= 15) CView.text("\n\n\"<i>Thanks for the business, " + mfn(player.gender, "laddie", "lassie", "customer") + "!  Remember, Niamh sells her Black Cat Beer every day from 8 until 4.</i>\"  You nod your head in thanks and step away from the table.");
    // 16:00 ending
    else {
        CView.text("\n\nThanks to your efforts, ");
        if (NiamhFlags.MET_NIAMH === 0) CView.text("the cat girl's");
        else CView.text("Niamh's");
        CView.text(" breasts have had the last of their beer squeezed from them.  She sighs in relief, caressing her shrunken breasts; while still hovering at around G-cup size, they're much smaller than they are when the day starts for her.  \"<i>Me thanks for the business; ye got the last mug for today.  Still, I'll be here tomorrow, full as ever.</i>\" She sighs softly.  \"<i>I regret to say that Niamh's Black Cat Beer doesn't look to be going out of business anytime soon.</i>\"  She stands and gathers her coat, slipping her arms into the sleeves.  The nimble girl draws the garment across her buxom chest, buttoning the slightly strained buttons with deceptive ease.  Dressed properly, she starts away, a bag full of gems bouncing against her swaying hips. You watch her go, staring at her back until she walks out the door.");
    }
    NiamhFlags.MET_NIAMH++;
    return { next: passTime(1) };
}

// [To Go]
function blackCatBeerToGo(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>That'll be two more gems, then,</i>\" the girl says.  \"<i>Though, if your hands get any more skillful I might have to pay you to take it,</i>\" she flirts.  You pass over two more gems, and she gratefully bags them and beams you a smile.");
    player.inventory.gems -= 2;
    ...
    NiamhFlags.MET_NIAMH++;
    // PC gains 1x BCB
    return player.inventory.items.createAdd(player, ConsumableName.BlackCatBeer, barTelAdre);
}

// Talk
function talkToNiamh(player: Character): NextScreenChoices {
    CView.clear();
    // (first time only)
    if (NiamhFlags.TALKED_NIAMH === 0) {
        CView.text("You ask the cat-girl to tell you about her past.");
        ...
        if (Time.hour < 15) CView.text("who usually sits over yonder ");
    ...
    if (player.inventory.gems >= 2)
        beer = getANiamhBeer;
    return { choices: [["Beer", beer], ["", undefined], ["", undefined], ["", undefined], ["Leave", passTime(1)]] };
}
// Leave
function leaveNiamh(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You decide that you don't really want to talk to this strange cat-girl and, as politely as possible, excuse yourself.");
...
// Black Cat Beer
// is affected by Black Cat Beer item effects
export function blackCatBeerEffects(player: Character, clearScreen: boolean = true, newLine: boolean = false) {
    if (clearScreen) CView.clear();
    if (newLine) CView.text("\n\n");
    CView.text("Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It's rich and sweet, and damn, but it's strong stuff!");
    ...
    player.stats.HP += 40 + randInt(21);
    let lib: number = 0;
    if (player.effects.has(EffectType.BlackCatBeer)) {
        if (100 - player.stats.lib >= 10) lib = 10;
        else lib = 100 - player.stats.lib;
        const effect = player.effects.getByName(EffectType.BlackCatBeer);
        if (effect) {
            effect.values.value1 += 4;
            effect.values.value2 += lib;
        }
        CView.text("\n\nDamn, it's even better with every extra drink!");
        player.stats.spe += -1;
        player.stats.int += -1;
        player.stats.lib += lib;
        player.stats.lust += 30 + randInt(player.stats.lib / 4);

    }
    else {
        if (100 - player.stats.lib >= 10) lib = 10;
        else lib = 100 - player.stats.lib;
        player.effects.create(EffectType.BlackCatBeer, { value1: 8, value2: lib });
        player.stats.spe += -5;
        player.stats.int += -5;
        player.stats.lib += lib;
        player.stats.lust += 20 + randInt(player.stats.lib / 4);
...
export function blackCatBeerExpires(player: Character) {
    const effect = player.effects.getByName(EffectType.BlackCatBeer);
    player.stats.spe += 4.5;
    player.stats.int += 4.5;
    player.stats.lib += (-1 * (effect ? effect.values.value2 : 0));

    player.effects.removeByName(EffectType.BlackCatBeer);
...
function giveNiamphBimboLiquer(player: Character): NextScreenChoices {
    return taintNiamh(player, true);
}

function giveNiamphSuccubiMilk(player: Character): NextScreenChoices {
    return taintNiamh(player, false);
}
...
function corruptOrBimboNiamhIntro(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("\"<i>Heya, favored customer,</i>\" the busty kitty greets as you approach.  \"<i>I have a favor to ask!  My drink's empty, and I get bored of me own brew,</i>\" Niamh explains.  \"<i>Would ya mind fetchin' me somethin', or are ye gonna force me to sample me own goods once more?</i>\"");
    ...
    if (player.inventory.items.has(ConsumableName.BimboLiqueur))
        CView.text("'bubbly' for her, in the form of the Bimbo Liqueur you got from the imp lord, or ");
    CView.text("maybe even a Succubus Milk?  You'll have to consider your options.  On the other hand, of course, something that corruptive would probably mutate her booze-filled boobs, most likely forcing her out of Tel'Adre altogether... and may fall back on your own head, to boot.  What do you do?");
    ...
    if (player.inventory.items.has(ConsumableName.BimboLiqueur)) choices[0] = ["Bimbo", giveNiamphBimboLiquer];
    if (player.inventory.items.has(ConsumableName.SuccubiMilk)) choices[1] = ["S.Milk", giveNiamphSuccubiMilk];
    choices[4] = ["Back", maybeLaterNiamh];

    return { choices };
}
// [Maybe Later]
export function maybeLaterNiamh(player: Character): NextScreenChoices {
    CView.clear();
...
export function taintNiamh(player: Character, bimbo: boolean = false): NextScreenChoices {
    CView.clear();
    CView.text("You grab Niamh's glass and one more and head off, making sure to turn a corner before commandeering an empty table.  Plunking the mugs down in front of you, you mutter to yourself as she moves away.");
    // [if sucmilk]
    if (!bimbo) {
    ...
    // [if bimbo liquor]
        ...
        NiamhFlags.NIAMH_STATUS = 2;
    }
    NiamhFlags.NIAMH_MOVED_OUT_COUNTER = 25;
    player.stats.cor += 10;
...
export function niamhPostTelAdreMoveOut(player: Character): NextScreenChoices {
    // Move her into the bazaar
    NiamhFlags.NIAMH_MOVED_OUT_COUNTER = -1;
    CView.clear();
    CView.text("Upon one of your routine searches, you're given pause as a half-coherent mess of a song comes tumbling across the grasses.  You move toward the cacophony of sound, fairly confident in what you will find.  Indeed, stumbling and shambling towards you comes a ");
    if (NiamhFlags.NIAMH_STATUS === 1)
        CView.text("demonic-looking");
    else
        CView.text("hourglass-shaped");
    CView.text(" Niamh, and it's obvious she's completely hammered.  ");
    if (NiamhFlags.NIAMH_STATUS === 1)
        CView.text("\"<i>W'hey there " + mf(player, "boyo", "big girl") + ", wanna...g'forrra goooo-gfh...time?</i>\"");
    else
        CView.text("\"<i>W'heeeey, honey!  I never thought I'd see yaaaagain!</i>\"");
    CView.text(" she slurs, struggling to stay upright between her impaired motor skills and her absolutely overfilled chest.  \"<i>Aaah've been drinkin' me 'shtash',</i>\" she explains, noticing your gaze.  \"<i>Canno' leddit go'tuh waste!</i>\" You nod your agreement, sneaking a glance behind her to observe the twin trails of ");
    if (NiamhFlags.NIAMH_STATUS === 1)
        CView.text("dark");
    else
        CView.text("light");
    CView.text(" fluid she's leaving behind.");
...
export function bimboChampagne(player: Character) {
    let bimboChampagneEffect = player.effects.getByName(EffectType.BimboChampagne);
    if (bimboChampagneEffect) {
        bimboChampagneEffect.values.value1 += 4;
        player.stats.spe += -2;
        player.stats.lib += 1;
        ...
        bimboChampagneEffect = player.effects.create(EffectType.BimboChampagne, { value1: 8, value2: 0, value3: 0, value4: 0 });
        // (Player has breasts smaller than DD-cup:
        if (player.body.chest.get(0)!.rating < 5) {
            ...
            bimboChampagneEffect.values.value2 = 5 - player.body.chest.sort(BreastRow.Largest).get(0)!.rating;
            player.body.chest.get(0)!.rating = 5;
        }
        // (Player does not have vagina:
        if (player.body.vaginas.length <= 0) {
            player.body.vaginas.add(new Vagina());
            CView.text("\n\nYou can feel ");
            if (player.body.cocks.length > 0) CView.text("the flesh under your cock[if (hasBalls = true)  and behind your [balls]]");
            else CView.text("the blank expanse of flesh that is your crotch");
            CView.text(" start to tingle and squirm... mmm... that feels nice!  There's a sensation you, like, can't describe, and then your crotch feels all wet... but in a good, sticky sorta way.  Oh, wow!  <b>You've, like, just grown a new virgin pussy!</b>  Awesome!");
            bimboChampagneEffect.values.value3 = 1;
        }
        // (player ass smaller than bimbo:
        if (player.body.butt.rating < 12) {
            ...
            bimboChampagneEffect.values.value4 = 12 - player.body.butt.rating;
            player.body.butt.rating = 12;
            if (player.body.hips.rating < 10) player.body.hips.rating = 10;
        }
        player.stats.spe += -10;
        player.stats.lib += 1;
...
export function removeBimboChampagne(player: Character) {
    ...
    if (bimboChampagneEffect.values.value2 > 0) {
        player.body.chest.get(0)!.rating -= bimboChampagneEffect.values.value2;
        CView.text("  As the trecherous brew fades, your [chest] loses some of its... bimboliciousness.  Your back feels so much lighter without the extra weight dragging down on it.");
    }
    if (bimboChampagneEffect.values.value3 > 0) {
        CView.text("  At the same time, your [vagina] slowly seals itself up, disappearing as quickly as it came.  Goodbye womanhood.");
        player.body.vaginas.clear();
    }
    if (bimboChampagneEffect.values.value4 > 0) {
        player.body.butt.rating -= bimboChampagneEffect.values.value4;
        CView.text("  Of course, the added junk in your trunk fades too, leaving you back to having a [butt].");
    }
    player.effects.removeByName(EffectType.BimboChampagne);
    CView.text("\n");
}

export function bazaarNiamh(player: Character): NextScreenChoices {
    CView.clear();
    // Bimbo Niamh:
    if (NiamhFlags.NIAMH_STATUS === 2) {
        if (NiamhFlags.TIMES_NIAMH_BAZAAR_MET === 0) {
            CView.text("As you enter the bazaar, you hear a familiar voice calls out to you. \"<i>Hey!  [name]!  Over here!  Like, it's me!</i>\"");
            CView.text("You follow the sound and find Niamh seated in a corner of the bazaar.  She is totally naked save for a small strip of cloth around her loins; her huge breasts lie exposed for all the world to admire.  She's surrounded by a small entourage of assorted beings; some are either hoping to earn a drink or else are simply here to ogle the massively buxom catgirl, others have evidently already drunk their fill, given their swollen bellies, feminine forms and platinum blonde coloration.  She grins as you approach.");
            CView.text("\"<i>Like, I was just roaming the plains, trying to, like, figure out where I was gonna go, and I found this place.  Isn't it, like, so neat here?</i>\"  She has apparently already forgotten that this was her destination all along!  \"<i>All these cool people who know how to have a totally good time!  Not like that dump in the desert, fulla buzzkills.  Yeah, I'm gonna, like, party all night and day here, [name].</i>\" She giggles yet again.  \"<i>But you're still, like, my number one customer.  So... you, like, wanna do something?</i>\" she asks.");
        }
        // [Bizarre Bazaar, repeat encounter]
            ...
            CView.text("Surrounded by blonde-haired, horny women, herms and traps, many of them sporting stomachs swollen with Niamh's copious champagne lactation, the bimbo catgirl enthusiastically waves you over.  \"<i>[name]!  Like, what can I do you for?</i>\" she giggles.");
        }
    }
    // Corrupt:
    else {
        // (General notification:
        CView.text("Given the pile of snoozing bodies, you can probably hazard a guess as to where Niamh decided to set up shop.");
        // (Approach:
        CView.text("Pushing your way past the drugged, sloshing, and fitful sleepers, you approach the encircled cat girl.  You catch her eye, and she waves you over.  \"<i>Welcome to me corner,</i>\" she snickers, glancing around at all of her victims.  \"<i>I don't suppose ya want a drink o' the brew?</i>\"  So saying, she lays her hands atop her ponderous bosom, leaning forward and smiling wickedly.");
    }
    NiamhFlags.TIMES_NIAMH_BAZAAR_MET++;
    ...
    if (player.body.cocks.length > 0)
        fuck = bazaarSex;
    return { choices: [["Get A Drink", drink], ["Tit-Fuck", fuck], ["", undefined], ["", undefined], ["Back", enterTheBazaar]] };
}

// [bimbo/corrupted beer drink texts]
function bazaardNiamhDrink(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.gems -= 2;
    CView.text("Approaching the fallen-from-grace (yet completely content) catgirl, you toss her a couple gems and request a glass of the house drink.");
    // Bimbo:
    if (NiamhFlags.NIAMH_STATUS === 2) {
        ...
        return { choices: [["Drink", drinkFromZeTap], ["Bottle", getBimboChampFromNiamh]] };
    }
    // Corrupt
    else {
        CView.text("Succubus milk Niamh Not implemented yet");
        return { next: passTime(1) };
    }
}
// {If player drinks from the tap:}
function drinkFromZeTap(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Your eyes lock onto her large, ever-dribbling nipples, the froth of spilt champagne caked on the table supporting her mammoth breasts, and you know you just have to have it from the source.  You push your way forward and seat yourself on a plush cushion that, from its many tears and suspicious-smelling stains, has seen better days, then grasp the huge boob in front of you, making the ditzy catgirl yowl in anticipation of pleasure.");
...
function getBimboChampFromNiamh(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You explain your need for a bottle of her champagne; the ditzy kitty takes several moments to register your words, then nods idly.  \"<i>Works fer me!</i>\" she responds, reaching behind her and rummaging in a large burlap bag.  She produces a flask from the sack's depths, unscrewing the cap and attempting to line it up with a nipple.  Try as she might, she just can't quite reach all the way, and with an airy giggle, she passes it to you instead.");
    ...
    return player.inventory.items.createAdd(player, ConsumableName.BimboChampagne, passTime(1));
}

// [Bazaar sex]
function bazaarSex(player: Character): NextScreenChoices {
    const y = player.body.cocks.indexOf(player.body.cocks.sort(Cock.Largest).get(1)!);
    CView.clear();
    if (NiamhFlags.NIAMH_STATUS === 2) {
        // Bimbo:Fuck a Bimbo Catgirl and Suck Dem Boozetits Like You Got a Pair (Or at least a dick; gotta have one of them)
        CView.text("As you approach the bimbofied cat-girl, your eyes wander lustily toward her huge, beer-lactating tits.  Seeing you stare, she giggles and shakes her chest enticingly, letting the huge, liquid-filled mounds wobble.  \"<i>Like, like what you see, [name]?</i>\" she purrs, her long black tail wagging excitedly behind her.");
    ...
    else if (NiamhFlags.NIAMH_STATUS === 1)	// Succubi Milk
    {
...
export function seanBimboBrewing(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  \"<i>I assume you'd like me to do something special with these,</i>\" he quips, somewhat dryly.");
...
function yeahSeanLetsBimbooze(player: Character): NextScreenChoices {
    ...
    player.inventory.items.consumeItem(ConsumableName.BimboChampagne);
    player.inventory.items.consumeItem(ConsumableName.BimboChampagne);
    player.inventory.items.consumeItem(ConsumableName.BimboChampagne);
    player.inventory.items.consumeItem(ConsumableName.BimboChampagne);
    player.inventory.items.consumeItem(ConsumableName.BimboChampagne);

    NiamhFlags.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER = 25;
...
// 24 hours later
export function getBimboozeFromSean(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Sean turns as you enter, managing a thin smile as he regards you.  \"<i>Excellent timing, I have just finished this!</i>\"");
    CView.text("He scoops a familiar-looking bottle from a tabletop, passing it off to you.  \"<i>Be somewhat careful with this,</i>\" he warns as you take it.  \"<i>The original's deleterious effect on the mind has been duplicated, I assure you.</i>\"\n\n");
    CView.text("You drop the vial into a pouch on your person.\n\n");
    // bimbo liqueur aqcquired
    NiamhFlags.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER = 0;
    return player.inventory.items.createAdd(player, ConsumableName.BimboLiqueur, incubusShop);
}

// [LEAVE]
function leaveWithBeerTits(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("The offer is tempting but right now you'd much rather deal with your boozy boobs privately.  You take off while trying to keep your [armor] modestly in place over your tits but it's difficult.  Your nipples constantly leak and drip a trail of alcohol all the way back to camp.  Thankfully by the time you arrive the effects seem to have mostly worn off.  Your nipples return to dripping milk, but although they've shrunk back down a bit they don't quite shrink all the way, leaving you with somewhat larger endowments than you had before.");
    growTopBreastRowDownwards(player, 2, player.body.chest.length);
    return { next: passTime(1) };
}
...
function sellYourBooze(player: Character): NextScreenChoices {
    CView.clear();
    player.inventory.gems += 2;
    let temp = Math.round(player.lactationQ() / 100);
    ...
    if (player.body.chest.length === 1) CView.text("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
    // If player has four or six breasts.
    else CView.text("Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it's obvious only the top pair of your tits can easily rest on the counter top.  \"<i>Lassie, it might be unconventional, but givin' the circumstance mayhaps it'd be best if ye laid down on the bar.</i>\"");
    ...
    if (player.body.vaginas.length > 0) CView.text("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
    player.stats.raw.lust += 10 + player.stats.sens / 5;

    // [If player has cocks]
    if (player.body.cocks.length > 0) CView.text("  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
    ...
    // If lust is high
        ...
        if (player.body.vaginas.length > 0) CView.text("You can feel your labia dripping with need.  ");
        if (player.body.cocks.length > 0) {
            CView.text("pre-cum drizzles down your " + describeCocksLight(player) + " as the need to fuck a pussy grows stronger.  A few wandering hands reveals some apparent interest amongst the folks in the bar in seeing your genitals as active as your nipples.");
        }
        // [FUCK THE BAR] [LEAVE]
        return { choices: [["Fuck Bar", barBeerOrgyTits], ["", undefined], ["", undefined], ["", undefined], ["Leave", leaveWithBeerTits]] };
    }
}
...
// First Paragraph
// ===============
function barBeerOrgyTits(player: Character): NextScreenChoices {
    CView.clear();
    // If [player has pussy]
    if (player.body.vaginas.length > 0) CView.text("The drooling of your [vagina] gets worse as the constant \"tapping of your kegs\" drives your sense of self-restraint to its limits.  The hands of various drunks start moving more aggressively over your breasts, eventually winding their way down between your thighs.");
    // IF [player has cock/s]
    if (player.body.cocks.length > 0) {
        CView.text("[EachCock] grows to its full length.  You try to keep ");
        if (player.body.cocks.length === 1) CView.text("it");
        else CView.text("them");
        CView.text(" discreet but it's impossible.  You try to push ");
        if (player.body.cocks.length === 1) CView.text("it");
        else CView.text("them");
        CView.text(" down or aside, but other pairs of hands begin stroking you.");
    }
    ...
    if (player.body.butt.rating >= 20) CView.text("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
    ...
    if (player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 20) CView.text("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
    CView.text("  Your [butt] hits the table first as the crowd puts you down.  Some patrons begin sucking on your nipples directly.");
    // [IF player has four or more giant tits, but not hugely pregnant]
    if ((player.pregnancyIncubation == 0 || player.pregnancyIncubation > 100) && player.body.chest.length > 1) CView.text("  You are essentially a pile of " + boozeBoobsType() + " filled tits.");
    // [IF player is hugely pregnant with four or more tits]
    else if (player.body.chest.length > 1) CView.text("  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");

    // ===============
    // Third Paragraph
    // ===============
    // IF player has pussy and Urta is in the bar and sex with her is unlocked.
    if (Time.hour < 15 && NiamhFlags.URTA_COMFORTABLE_WITH_OWN_BODY > 0 && player.body.vaginas.length > 0) {
        ...
        if (player.pregnancyIncubation === 0) CView.text("  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.");
    ...
    // ELSE IF player has pussy and Urta is not in the bar.
        ...
        if (player.vaginalCapacity() < 60) CView.text("  One is sporting a cock that is obviously too massive for your pussy, and eventually backs down so that the other one, despite still being on the large size, can take the honor of fucking your cunt silly.");
        // ELSE IF [player has a gaping pussy or bigger]
        else CView.text("  They're both sporting large canine cocks, but one is particularly massive while the other would likely be a loose fit for a pussy as naturally stretched out as yours.  The largest one wins out and positions himself to give your overly wide fuck hole a much needed stretching.");
    ...
    if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 100) CView.text("  \"<i>Oi!  Careful folks, this babe could blow at any time!</i>\"  Niamh pats your belly with chiding affection.");
    ...
    if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + numToCardinalText(player.body.chest.reduce(BreastRow.TotalNipples, 0)) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
    // ELSE[Character doesn't have fuckable nipples]
    else CView.text("  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh's breasts.");
    ...
    if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("  The cocks filling your nipples start pumping hot sperm into your breasts, mixing with your " + boozeBoobsType() + " and causing every nerve ending in your nipples to tingle.  Some of them ejaculate so much that it feels as if your breasts are swelling even larger, almost to the point of bursting before the cum spills out.");
    // IF [Player has cocks]
    if (player.body.cocks.length > 0) {
        CView.text("  You feel as if your body is exploding everywhere.  Niamh gags momentarily as her mouth fills with your seed, and you realize your cock");
        if (player.body.cocks.length > 1) CView.text("s have");
        else CView.text(" has");
        CView.text(" begun spurting ");
        if (player.body.cocks.length === 1) CView.text("its ");
        else CView.text("their ");
        CView.text("jizz.  Niamh swallows it down her throat but not without some of it getting plastered across her face.");
        // IF [character is pregnant]
        if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) CView.text("  Globs of cum splatter across your gravid belly.");
    }
    ...
    if (player.effects.getByName(EffectType.Edryn).value1 >= 5 && player.gender === 3 && Time.hour === 14 && NiamhFlags.URTA_COMFORTABLE_WITH_OWN_BODY > 0) {
        ...
        // IF [player has a single cock]
        if (player.body.cocks.length === 1) CView.text("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
        // If [player has multiple cocks]
        else if (player.body.cocks.length > 1) CView.text("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + describeCocksLight(player) + ".");
        ...
        if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) CView.text("  Your pregnant belly presses up against Edryn's ass as both Urta's cock and Edryn's pussy keeps you pinned in place between them.");
        // IF[player is not pregnant]
        else CView.text("  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass.");
        ...
        if (player.body.chest.length === 2) CView.text("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
        // IF [player has six giant breasts]
        else if (player.body.chest.length === 3) CView.text("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
        // IF [player has large balls]
        if (player.body.balls.count > 0 && player.body.balls.size > 14) CView.text("  Your hanging testicles bounce around between everyone's legs.");
        // IF [player has giant ass]
        if (player.body.butt.rating >= 22) CView.text("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
    ...
    // IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy]
    else if (player.body.vaginas.length > 0 && Time.hour < 15 && NiamhFlags.URTA_COMFORTABLE_WITH_OWN_BODY > 0) {
        ...
        if (player.body.chest.length > 1) CView.text("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
        // If [player not pregnant]
        if (player.pregnancyIncubation == 0) CView.text("  Urta's small ocean of sperm streams out from your pussy like a river down your legs as you try to stand.");
    ...
    // Generic ending if the first two don't trigger
        ...
        // IF [player has pussy]
        if (player.body.vaginas.length > 0) CView.text("  The giant knot of the dog morph finally breaks its way inside causing your pussy to become overstretched.  The pleasurable torture only gets worse as he begins unloading a river of his cum inside you.  It begins squirting out around his cock and soaks both of your thighs in the process.");
        displayStretchVagina(player, 60, true, true, false);

        CView.text("  Niamh finally has her fill and dismounts you, but no sooner has her pussy left your face than a determined cock belonging to some sort of cat morph fills your mouth.  Your eyes bug out as it thrusts down your throat.");
        // If [player has pussy]
        if (player.body.vaginas.length > 0) CView.text("  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that's been causing your tits to bounce all over the place.");

        // IF [player has single cock small-med cock]
        if (player.body.cocks.length === 1 && player.body.cocks.get(0)!.area < 75) CView.text("\n\nA cat girl with six C-cup breasts jumps up onto the table and mounts you.  She grabs your cock and proceeds to shove it up between the folds of her tight pussy.");
        ...
        if (player.body.cocks.length === 1) CView.text("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
        // IF [player has multiple cocks]
        else if (player.body.cocks.length > 1) CView.text("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
        // IF [player has pussy]
        if (player.body.vaginas.length > 0) {
            CView.text("  You walk slightly bow legged out of the bar.  Cum is still dripping from your snatch and will likely continue to do so for a while.");
            // IF [player is not pregnant]
            if (player.pregnancyType === 0) CView.text("  You can't help but wonder how virile those dog morphs might have been as their cum and the cum of other customers sloshes around inside your uterus.");
            const temp = randInt(6);
            switch (temp) {
                case 0:
                    attemptKnockUp(player, PregnancyType.MINOTAUR, IncubationTime.MINOTAUR);
                    break;
                case 1:
                    attemptKnockUp(player, PregnancyType.MOUSE, IncubationTime.MOUSE);
                    break;
                case 2:
                    attemptKnockUp(player, PregnancyType.MOUSE, IncubationTime.MOUSE); // I'm betting this was meant to be dog morph chance
                    break;
                default:
                    attemptKnockUp(player, PregnancyType.CENTAUR, IncubationTime.CENTAUR);
                    break;
            }
        }
        growTopBreastRowDownwards(player, 2, player.body.chest.length);
