...
export const FrostyFlags = Flags.register("Frosty", {
    MET_FROSTY: 0,
    SIGNED_FROSTYS_CONTRACT: 0,
    FROSTY_POINTS: 0,
    SEEN_GENDERLESS_FROSTY_REJECTION: 0,
});
...
export function approachFrosty(player: Character): NextScreenChoices {
    ...
    if (FrostyFlags.MET_FROSTY === 0) {
        ...
        return { yes: yesFrostyLetsHaveSexForCupcakesWhoThoughtThisWasAGoodIdeaIMeanSeriously, no: noContractForMeSloot };
    }
    else {
        // [Picking the Frosty button at the Bakery]
        CView.text("You go out to Frosty’s stand; she’s there manning it with a huge smile on her face. “<i>Hello there customer, how may I help you?</i>” You greet her with your own smile and hello and take a look at her menu:\n");
        CView.text("1. Cupcake x 1 = 15gems\n");
        CView.text("2. Cupcake x 5 = 60gems\n");
        CView.text("3. Cupcakes x 10 = 110gems\n");
        CView.text("4. Cupcakes x 20 = 200gems\n");
        ...
        Button 0 [Leave]*/
        return frostyMainMenu(player);
    }
}

export function frostyMainMenu(player: Character): NextScreenChoices {

    const choices: ScreenChoice[] = [];
    if (player.inventory.gems >= 15) choices[0] = ["x1", choiceWrap(cupcakeBuy, 1)];
    if (player.inventory.gems >= 60) choices[1] = ["x5", choiceWrap(cupcakeBuy, 5)];
    if (player.inventory.gems >= 110) choices[2] = ["x10", choiceWrap(cupcakeBuy, 10)];
    if (player.inventory.gems >= 200) choices[3] = ["x20", choiceWrap(cupcakeBuy, 20)];
    if (FrostyFlags.SIGNED_FROSTYS_CONTRACT === 0) choices[8] = ["Contract", getAFuckingFuckContractFromFucks];
    else choices[8] = ["Specials", frostysLimitedSpecialMenu];
    choices[9] = ["Leave", telAdreMenu];

    return { choices };
}

export function frostySpecialsMenu(player: Character, free: boolean = false): NextScreenChoices {
    // 1. Hand – 5. points
    // 2. Mouth-15. Points

    if (!free && FrostyFlags.FROSTY_POINTS < 5)
        CView.text("\n\nYou don't have enough points for any services.");
    else if (!free && FrostyFlags.FROSTY_POINTS < 15)
        CView.text("\n\nYou don't have enough points for mouth service.");

    const choices: ScreenChoice[] = [];
    if (free || FrostyFlags.FROSTY_POINTS >= 5) choices[0] = ["Hand", frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp];
    if (free || FrostyFlags.FROSTY_POINTS >= 15) choices[1] = ["Mouth", useFrostysMouth];
    if (free || FrostyFlags.FROSTY_POINTS >= 1) choices[8] = ["EatHerOut", cunnilingateFrosty];
    choices[9] = ["Back", approachFrosty];

    return { choices };
}

export function cupcakeBuy(player: Character, price: number = 1): NextScreenChoices {
    CView.clear();
    CView.text("You make your purchase and swiftly devour the results. They're surprisingly delicious!");
    // 15
    if (price === 1) player.inventory.gems -= 15;
    // 60
    if (price === 5) player.inventory.gems -= 60;
    // 110
    if (price === 10) player.inventory.gems -= 110;
    // 200
    if (price === 20) player.inventory.gems -= 200;
    frostyPoints(price);

    return { next: approachFrosty };
}

// [Yes]
export function yesFrostyLetsHaveSexForCupcakesWhoThoughtThisWasAGoodIdeaIMeanSeriously(player: Character): NextScreenChoices {
    CView.clear();
    FrostyFlags.SIGNED_FROSTYS_CONTRACT = 1;
    CView.text("Being rewarded with sex for buying cupcakes, what could be better? You take the pen from Frosty’s hand and ask where to sign. She lets out a joyful “<i>YAY!</i>” and does a little jump in the air. She zooms back behind the stand and points at the lines where your name, last name, initials, date of birth, mother’s maiden name and even your pet's name should be written. You sign all these areas until you’re finally on the last page of the contract. Thank goodness too, your hand is beginning to cramp up. You put the pen down and rub your wrist. Frosty spins the stack a 180° with the tip of her finger, does a quick flipping through the papers, nods her head, puts the stack in a lock-box under the counter and pops back up.");
    ...
    return { yes: yesIWantYourFreeSampleYouFuckingDiseasedCupcakeSlut, no: noIDontWantAFreebieDiseaseYouSlut };
}

// [Yes]
export function yesIWantYourFreeSampleYouFuckingDiseasedCupcakeSlut(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You follow Frosty to her booth. She goes behind the counter and spins the menu with her fingertip and stops it with the palm of her hand. She goes back to her business while you look over the menu.  The lettering and style of it has changed into something more appealing and seductive.");
    // [Specials Menu is shown]
    return frostySpecialsMenu(player, true);
}

// [No]
export function noIDontWantAFreebieDiseaseYouSlut(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You’re too busy right now, so you deny the free service. “<i>Okeydokey, see you around, [name]!</i>” Frosty does a twirl on one foot and runs back to her stand. Such a nice girl.");

    return { next: telAdreMenu };
}

// [No](for the contract)
export function noContractForMeSloot(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Something about signing a contract doesn’t seem right to you, especially one for cupcakes. You shake your head no; she looks a bit sad about being rejected but quickly perks back up.");
...
export function getAFuckingFuckContractFromFucks(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Frosty zooms around the counter, appearing next to you like a pink blur “<i>OH! You changed your mind!?</i>” she asks excitedly with her hands held up to her face like she’s praying.");
    // [Yes [No]
    // [Yes]
    // [Go to the “Yes” section when offered the contract the first time.]

    return { yes: yesFrostyLetsHaveSexForCupcakesWhoThoughtThisWasAGoodIdeaIMeanSeriously, no: noIDontWantToSignYourFuckingContractGoddamned };
}

// [No]
export function noIDontWantToSignYourFuckingContractGoddamned(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Frosty lets out a discouraged “<i>Oh</i>” and slowly walks back to her side of the counter.");
    // [Back at Frosty’s Main menu]
    return frostyMainMenu(player);
}

// [Specials](If the player has signed the contract)
export function frostysLimitedSpecialMenu(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Frosty spins the menu with a fingertip and stops it with the palm of her hand. She goes back to her business while you look over the menu.  The lettering and style of it has changed into something more appealing and seductive:\n\n1. Hand – 5 points\n2. Mouth - 15 points\nSPECIAL: Eat Me Out - 1 point\n\n<b>Current points: " + frostyPoints() + "</b>");
    return frostySpecialsMenu(player);
}

function frostyPoints(arg: number = 0): number {
    if (arg === 0) return FrostyFlags.FROSTY_POINTS;
    // If any change, add it in.
    FrostyFlags.FROSTY_POINTS += arg;
    // Make sure it aint negative
    if (FrostyFlags.FROSTY_POINTS < 0) FrostyFlags.FROSTY_POINTS = 0;
    return FrostyFlags.FROSTY_POINTS;
}

// [Hands]
export function frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp(player: Character): NextScreenChoices {
    CView.clear();
    if (player.body.cocks.length <= 0 && player.body.vaginas.length <= 0) {
        return genderlessCatchAllForFrosty(player);
    }
    if (player.body.cocks.length > 0 && player.body.vaginas.length <= 0) {
        CView.text("“<i>Okay, one tug-and-pull comin' right up.</i>” she giggles at her own name for a hand-job and ducks under the table, pulling up a big, fancy, cushioned chair that looks like it belongs to a noble. It barely fits in the tiny space under her booth. She throws the chair right next to her stand and it lands perfectly, hardly making a sound. In one moment Frosty disappears and reappears on the chair, sitting on its arm and waving her hand over to the seat, gesturing for you to sit. You follow her gesture and take a seat in the comfy chair. Just sitting there makes you feel like the ruler of a land. Placing your arms on the armrests, you feel like a boss.");
        ...
        CView.text("\n\n“<i>Ooooh, your ");
        if (player.body.cocks.sort(Cock.Largest).get(0)!.length < 7) CView.text("little");
        else CView.text("big");
        CView.text(" friend is just waiting for me to milk his cream out. Tee-hee.</i>”");
    ...
    // [If player is Herm]
        ...
        CView.text("“<i>Okay, one stroke-and-poke comin’ right up</i>” she snickers at her own name for using her hand to pleasure a herm. She ducks under the table and pulls up a big, fancy, cushioned chair that looks like it belongs to a noble and also shouldn’t be able to fit in the tiny space under her booth.");
...
// [Mouth]
export function useFrostysMouth(player: Character): NextScreenChoices {
    if (player.body.cocks.length <= 0 && player.body.vaginas.length <= 0) {
        return genderlessCatchAllForFrosty(player);
    }
    CView.clear();
    ...
    // [If player is Herm]
        ...
        CView.text("“<i>One banana split sundae special coming right up.</i>” You roll your eyes at the comment as Frosty blurs away her display then vaults over the counter with ease and is suddenly behind you. You quickly turn around just as she pushes your backside against her stand and falls to her knees. She’s about to pull the lower half of your [armor] off but decides not to. You wonder what she’s going to do as she stretches her jaw and sticks out her tongue for a few minutes. After she’s done with that, she cracks her neck and looks at your pants, like it’s a challenge. Then, in an instant, she’s able to undo your [armor] with just her tongue and teeth and has you standing waist-down naked. Now you have second thoughts of having any of your naughty bits near that mouth.");
...
export function genderlessCatchAllForFrosty(player: Character): NextScreenChoices {
    ...
    // [If player selects any option besides sex and is Gender-less repeatable]
        ...
        CView.text("“<i>Sorry customer, you need a cock or a pussy or BOTH if I’m to do that!</i>” she sings in a cheesy jingle tone that makes you go, “<i>ughhhhhh.</i>”");
    }
    // [Back to Frosty’s Special Menu.]
    return frostySpecialsMenu(player);
}
...
function analWithACupcakeSloot(): void
{
    ...
    {
        CView.text("“<i>So you wanna be the pitcher or the catcher?</i>” she ask with devilish smile. What the hell is that suppose to mean?");
        //[pitcher?]  [catcher?]
    }
    else
    {
...
It moves only few inches on your onsides. Your mind starts racing with pleasure as Frosty’s other hand wanders over your [allBreastsDescript] and pinches on your [describeNipple(player, player.body.chest.get(0))], pulling and twisting the nip and taking nice hard grabs at your [ describeBreastRow(player.body.chest.get(0))] A familiar build-up forms down in your crotch, you start pushing back against Frosty’s “cock” to hurry up the climax and start pinching your other [describeNipple(player, player.body.chest.get(0))] minicing the same action as the torturous nymph does, abusing the sensitive bud all in the name of sexaul pleasure.
