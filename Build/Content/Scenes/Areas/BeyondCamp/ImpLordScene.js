define(["require", "exports", "Engine/Display/ScreenDisplay", "Engine/Display/ContentView", "Engine/Combat/CombatManager", "Content/Scenes/PassTime", "Engine/Utilities/SMath", "Content/Descriptors/CockDescriptor", "Engine/Body/Cock", "Content/Effects/EffectType", "Engine/Body/BreastRow", "Content/Descriptors/VaginaDescriptor", "Content/Modifiers/VaginaModifier", "Engine/Body/Legs", "Content/Modifiers/ButtModifier", "Content/Descriptors/BreastDescriptor", "Content/Modifiers/BreastModifier", "Content/Descriptors/ButtDescriptor", "./ImpLord", "Content/Combat/Encounter"], function (require, exports, ScreenDisplay_1, ContentView_1, CombatManager_1, PassTime_1, SMath_1, CockDescriptor_1, Cock_1, EffectType_1, BreastRow_1, VaginaDescriptor_1, VaginaModifier_1, Legs_1, ButtModifier_1, BreastDescriptor_1, BreastModifier_1, ButtDescriptor_1, ImpLord_1, Encounter_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    // IMP LORD
    function impLordEncounter(player) {
        ContentView_1.CView.clear();
        ContentView_1.CView.text("A large corrupted imp crosses your path. He flashes a cruel smile your way.  No way around it, you ready your " + player.inventory.weapon.displayName + " for the fight.");
        return CombatManager_1.CombatManager.beginBattle(new Encounter_1.Encounter(player, new ImpLord_1.ImpLord()));
    }
    exports.impLordEncounter = impLordEncounter;
    // Rewards
    // +20 XP
    // +7-15 Gems
    // Common Drops: Imp Food & Incubus Draft
    // Rare Drops: LaBova & Minotaur Blood
    function defeatImpLord(player, impLord) {
        ContentView_1.CView.clear();
        if (impLord.stats.HP < 1) {
            ContentView_1.CView.text("The greater imp falls to the ground panting and growling in anger.  He quickly submits however, the thoroughness of his defeat obvious.  You walk towards the imp who gives one last defiant snarl before slipping into unconsciousness.");
            return { next: PassTime_1.passTime(1) };
        }
        else {
            ContentView_1.CView.text("The muscular imp groans in pained arousal, his loincloth being pushed to the side by his thick, powerful dick.  Grabbing the useless clothing, he rips it from his body, discarding it.  The imp's eyes lock on his cock as he becomes completely ignorant of your presence.  His now insatiable lust has completely clouded his judgment.  Wrapping both of his hands around his pulsing member he begins to masturbate furiously, attempting to relieve the pressure you've caused.");
            // Leave // Rape]
            const choices = [];
            if (player.stats.lust >= 33)
                choices[0] = ["Sex", ScreenDisplay_1.choiceWrap(sexAnImpLord, impLord)];
            choices[9] = ["Leave", PassTime_1.passTime(1)];
            return { choices };
        }
    }
    exports.defeatImpLord = defeatImpLord;
    function loseToAnImpLord(player, impLord) {
        ContentView_1.CView.clear();
        if (player.body.vaginas.length > 0 && (player.gender === 2 || SMath_1.randInt(2) === 0))
            return getRapedAsAGirl(player, impLord);
        else if (player.body.cocks.length > 0)
            return loseToImpLord(player, impLord);
        else {
            ContentView_1.CView.text("Taking a look at your defeated form, the imp lord snarls, \"<i>Useless,</i>\" before kicking you in the head, knocking you out cold.");
            player.stats.HP -= 9999;
            return { next: PassTime_1.passTime(1) };
        }
    }
    exports.loseToAnImpLord = loseToAnImpLord;
    // Rape
    function sexAnImpLord(player, impLord) {
        ContentView_1.CView.clear();
        ContentView_1.CView.text("You grin evilly and walk towards the defeated corrupted creature.  He doesn't take notice of you even though you're only inches away from him.  You remove your [armor] slowly, enjoying the show the imp is giving you.  But soon it's time for you to have fun too.");
        // (No line break)
        // if(player doesn't have centaur legs)
        if (!player.body.legs.isTaur())
            ContentView_1.CView.text("  You grab his hands, removing them from his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + ". This gets his attention immediately, and you grin widely, pinning him to the ground.");
        else
            ContentView_1.CView.text("  You place one of your front hooves on his chest, knocking him onto his back.  He attempts to get back up, but you apply more pressure to his thick, manly chest, until he gasps.  The imp gets the idea quickly and stops masturbating, all of his focus now on you.");
        // Continues in, Male Anal, Female Vaginal, or Breastfeed
        const choices = [];
        choices[9] = ["Leave", PassTime_1.passTime(1)];
        if (player.stats.lust >= 33) {
            if (player.body.cocks.length > 0 && player.body.cocks.find(Cock_1.Cock.CockThatFits(impLord.analCapacity())))
                choices[0] = ["FuckHisAss", ScreenDisplay_1.choiceWrap(impLordBumPlug, impLord)];
            if (player.body.cocks.length > 0)
                choices[1] = ["Get Blown", getBlownByAnImpLord];
            if (player.body.vaginas.length > 0)
                choices[2] = ["Ride Cock", ScreenDisplay_1.choiceWrap(femaleVagRape, impLord)];
            if (player.effects.has(EffectType_1.EffectType.Feeder))
                choices[3] = ["Breastfeed", ScreenDisplay_1.choiceWrap(feederBreastfeedRape, impLord)];
        }
        return { choices };
    }
    // MALE ANAL
    function impLordBumPlug(player, impLord) {
        ContentView_1.CView.clear();
        ContentView_1.CView.image("implord-win-male-fuck");
        let cockThatFits = player.body.cocks.find(Cock_1.Cock.CockThatFits(impLord.analCapacity()));
        if (!cockThatFits)
            cockThatFits = player.body.cocks.sort(Cock_1.Cock.Smallest).get(0);
        ContentView_1.CView.text("You grab the muscular creature by one of his long pointed ears, pulling him to his feet. He protests slightly, and gives a slightly defiant snarl of discomfort.  Lucky for him you're in a forgiving mood and ignore his whining.");
        // (No line break)
        if (player.body.tallness < 72) {
            ContentView_1.CView.text("  You give a powerful shove and push the imp to his knees.");
        }
        ContentView_1.CView.text("  You pull the imp's head towards your [hips], forcing his lips against the base of your " + CockDescriptor_1.describeCock(player, cockThatFits) + ".  The imp quickly gets the idea and begins to lick and suckle at your " + CockDescriptor_1.describeCockHead(cockThatFits) + " expertly.");
        ContentView_1.CView.text("\n\nYou pet the top of his smooth head encouragingly, his tongue quickly soaking your length in saliva.  With little encouragement, the imp begins to take your " + CockDescriptor_1.describeCock(player, cockThatFits) + " into his mouth, focusing on milking the head of its delicious precum.  You soon remember what you'd intended to do with the little cock slut, and push him away from your length.  You could swear the imp whimpered in response to this, which makes you grin.");
        ...
        ContentView_1.CView.text("\n\nYou drape your " + CockDescriptor_1.describeCock(player, cockThatFits) + " between the imp's butt cheeks.  Teasingly you begin thrusting between the muscular cleft of his ass, much to the creature's dismay.  The greater imp whines submissively, desperate for you to enter him.  His " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " drools as you tease his ass, creating a small puddle of pre between his hooves.  The poor creature becomes so desperate that he reaches back and spreads his muscular cheeks with his hands.  You catch a small glimpse of his now gaping hole as your cock continues to slide between his cheeks.  You tease him with one last empty thrust, before grabbing his shoulders and forcing your " + CockDescriptor_1.describeCock(player, cockThatFits) + " deep inside the poor creature.");
        ContentView_1.CView.text("\n\nThe imp loses control immediately and nearly collapses from the massive orgasm.  His large, corruption-bloated balls clench up, and semen floods out of his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " like a fountain.  The hot demon seed puddles around his feet, soaking the ground in his thick boy-goo.");
        ContentView_1.CView.text("\n\nHis orgasm also cause his anus to tighten and spasm around your " + CockDescriptor_1.describeCock(player, cockThatFits) + ", as if he was milking you of your seed.  Unable to resist, you start plunging yourself rapidly in and out of the spasming hole.");
        // (No line break)
        if (player.body.balls.count > 0) {
            ContentView_1.CView.text("  You feel the slapping of your [balls] against the imp's, a sensation that only spurs you on, causing you to slam into him over and over wildly.");
        }
        if (player.body.chest.sort(BreastRow_1.BreastRow.Largest).get(0).rating >= 1 && player.lactationQ() >= 200) {
            ContentView_1.CView.text("  You give a desperate groan, and grab one of your [chest] roughly.  You pinch and massage your ");
            if (player.body.chest.reduce(BreastRow_1.BreastRow.AverageNipplesPerBreast, 0) > 1)
                ContentView_1.CView.text("[nipples]");
            else
                ContentView_1.CView.text("[nipple]");
            ContentView_1.CView.text(", making you moan in ecstasy and leak milk, quickly soaking your chest.");
        }
        // if(player has Fertility perk && player.body.balls.count >= 4)
        if (player.body.balls.count >= 4) {
            ContentView_1.CView.text("\n\nAs much as you would love to continue the pleasure you can't last any longer.  You howl in intense pleasure and cum.  Your [balls] tighten against your body, and empty their contents. Your " + CockDescriptor_1.describeCock(player, cockThatFits) + " pulses and spasms, releasing wave and wave of semen into the greater imp's belly.  The pleasure is intense, and almost painful as your cum.");
            // if(player is a herm)
            if (player.gender === 3)
                ContentView_1.CView.text("\n\nAs your " + CockDescriptor_1.describeCock(player, cockThatFits) + " erupts, your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " tenses up tightly, spasming, desperate to be filled.  After a moment your girl juice begins to soak your inner thighs.  Your legs begin to tremble from the intensity of it all, and you question if you'll be able to make it back to your camp after this.");
            ContentView_1.CView.text("\n\nThe imp howls as his ass is flooded with cum.  His stomach begins to expand from the sheer amount of fluid you pump into him, and the sensation of being over filled causes the poor creature to cum a second time, spilling more semen into the already massive puddle.  His " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " pumps wave after wave of corrupt seed onto the ground, soaking his hooves even further.");
            ContentView_1.CView.text("\n\nYou're sure you must have blacked out at some point as you feel the last of your seed force its way out of your " + CockDescriptor_1.describeCock(player, cockThatFits) + " and into the imp's demon belly.  You wobble slightly and lean down, grabbing the ragged remains of the imp's loincloth.  Weakly you pull out of the demon's hole, and quickly stuff the cloth in its place as a makeshift plug.");
    ...
    // FEMALE VAGINAL
    function femaleVagRape(player, impLord) {
        ContentView_1.CView.clear();
        ContentView_1.CView.image("implord-win-female-fuck");
        ContentView_1.CView.text("With little ceremony you grab the imp's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " and begin to jerk your hand up and down roughly.  The little muscular beast begins to whine loudly, in protest to the rough and likely painful mistreatment of his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + ".  In spite of the protests, the rough treatment goes over well, as the creature begins to leak hot demon pre across your hand, which you smear across the shaft as a natural hot lube.");
        ContentView_1.CView.text("\n\nLicking your lips, you squat above the little demon, positioning your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " above the thick log of meat.  You stay still for a moment, and question if this was a good idea.  The demon was so thick you hadn't even been able to fit your hand around his shaft.  There was little chance you'd get out of this without some rather rough stretching, but the scent of the demon's arousal, and your corrupt lust spur you onwards.");
        ContentView_1.CView.text("\n\nYou lower yourself, slowly, watching as the head of the demon's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " begins to spread your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + ".  You begin to pant as you're stretched wide; you can hardly believe you can take him.  It takes you several minutes of steady, shallow, downward thrusts before the imp is completely hilted inside of your body.  You sit still for a moment to adjust to the intense sensation. The muscular creature doesn't seem to mind, as his tongue is hanging slightly out of the side of his mouth, panting like a dog.");
        VaginaModifier_1.displayStretchVagina(player, impLord.body.cocks.get(0).area, true, true, false);
        ContentView_1.CView.text("\n\nAs you adjust, you get a devilish idea.  Both your hands gently begin to massage the imp's muscular abs and chest; before taking each of his fertite-pierced nipples between your fingers.  You pinch and tug upwards, roughly playing with his sensitive nipples.  The imp instinctively bucks his hips, slamming into you roughly, making you yelp and clench your [vagina] around his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + ".");
        ...
        if (player.body.cocks.length === 1) {
            if (player.body.cocks.get(0).length < 8) {
                ContentView_1.CView.text("\n\nThe intense stimulation causes your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " to begin leaking pre across the imp's stomach.  You remove one hand from the imp's nipples to pay attention to your aching " + CockDescriptor_1.nounCock(Cock_1.CockType.HUMAN) + ".  As you begin jerking yourself off, more and more pre puddles on the imp's abs.  Your pre begins to pool in and overflow from the imp's tight belly button.");
            }
            else if (player.body.cocks.get(0).length <= 16) {
                ContentView_1.CView.text("\n\nThe intense stimulation causes your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " to begin leaking pre across the imp's chest.  You remove one hand from the imp's nipples to pay attention to your aching " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + ".  As you begin jerking yourself off, more and more pre drips across the imp's aching nipples, your pre a warm relief for the tender abused teats.  The warmth of your juice across his chest makes the little demon moan and buck harder into your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + ".");
            }
            else {
                ContentView_1.CView.text("\n\nAs your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " is pounded relentlessly, your rock hard " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " slaps the imp in the face, each time leaving another trail of pre in its wake.  The imp's arousal only intensifies from this, and he's soon lapping and suckling at your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + ", expertly trying to get you off.");
                ContentView_1.CView.text("\n\nIt's not long before the imp's face is soaked in your pre, and thanks to the imp's oral ministrations your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " is now leaking pre continually. It's only a matter of time before you blow now.");
            }
        }
        else if (player.body.cocks.length > 1) {
            ContentView_1.CView.text("\n\nAs you continue your cruel torture of the imp's now bruised nipples, he reaches up both of his hands and takes hold of your " + CockDescriptor_1.describeCocksLight(player) + ".  The imp feverishly masturbates your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " and " + CockDescriptor_1.describeCock(player, player.body.cocks.get(1)) + " as he thrusts into your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + ".  The intense stretching of your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " by the " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " sends waves of pleasure coursing through you, straight into your " + CockDescriptor_1.describeCocksLight(player) + ", which are now leaking precum profusely onto the imp's muscular body.");
        }
        ContentView_1.CView.text("\n\nYou give one final rough tug on the imp's nipples, and that's all he can handle.  His cock spasms inside your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " and erupts, sending wave after wave of hot, demon cum into your womb. His tainted testes spasm in their taut sack, continuing to pour their contents into you, until your belly is swollen and full of the hot seed.  You gasp in pleasure, unable to fight the pleasure of being filled so thoroughly.  Your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " spasms, and you clench around the " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " inside of you, as your girl juices begin to flow out of you with your earth shattering orgasm.");
        if (player.body.cocks.length === 1) {
            ContentView_1.CView.text("\n\nWith your orgasm, your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " erupts, sending waves of boy goo across the imp's already drenched body.  The imp mewls as your seed spills across his masculine body, marking him as the bitch he is.  He leans forward and takes your cock into his mouth, suckling what's left of your orgasm out of you.  The imp drinks down all the cum he can. His mouth feels so hot, you don't really want it to end, but soon the orgasm settles down and you come back to reality from your lust induced euphoria.");
            ...
            ContentView_1.CView.text("\n\nAs you ride out your orgasm the crafty imp pulls your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " and " + CockDescriptor_1.describeCock(player, player.body.cocks.get(1)) + " towards his mouth. Locking his lips around the tips of your two cocks, he suckles down every last drop of your jizz they offer.  You begin to mewl and whine in desperation as your orgasm seems to last an eternity.  The imp's skilled tongue and cock manage to work all of your favorite and most sensitive spots, sending you into complete euphoria.  Once your orgasm begins to settle the imp pulls back allowing the last few strings of semen to splatter across his face.");
        }
        ContentView_1.CView.text("\n\nAfter a few moments of recovery, you slowly lift yourself off the imp.  Cum rushes out of your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " and you clamp your muscles down as best as you can to keep the warm substance inside of you.  You give your swollen, cum-filled belly a motherly rub, before gathering your [armor].");
        player.orgasm();
        player.stats.cor += 1;
        player.slimeFeed();
        return { next: PassTime_1.passTime(1) };
    ...
    function feederBreastfeedRape(player, impLord) {
        ContentView_1.CView.clear();
        ContentView_1.CView.image("implord-win-female-breastfeed");
        ...
        if (player.body.tallness > 48 && player.body.tallness < 60 && player.body.legs.type !== Legs_1.LegType.CENTAUR) {
            ...
            if (player.body.cocks.length > 0) {
                ContentView_1.CView.text("  [EachCock] twitches violently from the stimulation of your [asshole].  Pre-cum begins to dribble out of your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + ".  You can't help but pant desperately as the warm pleasurable sensation of arousal fills your whole being.");
            }
            if (player.body.vaginas.length > 0) {
                ContentView_1.CView.text("  The intense stimulation of your [chest] is causing your [vagina] to become wet with girl juice... so much so that your femcum has started to seep down your taint towards your ass.  It's probably a good thing, as it's now become a lubricant for the greater imp's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + ".");
            }
            ...
            if (player.body.chest.length === 1)
                ...
                ContentView_1.CView.text("\n\nYou chuckle at the still very hungry imp, and begin massaging your second row of " + BreastDescriptor_1.describeBreastRow(player.body.chest.get(1)) + ", occasionally pinching your " + BreastDescriptor_1.describeNipple(player, player.body.chest.get(1)) + "s, drawing a few beads of milk from them.  The imp makes short work of your first row of breasts and has moved towards your second.");
                ...
                ContentView_1.CView.text("\n\nNodding your acceptance, he continues his work much more carefully.  He's taking his time again instead of just sucking wildly.  You reach down curiously, and tug on the imp's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " but find that it's shrinking.  As you hold it, it shrinks more and more. You wonder what will happen to him if he continues to nurse.");
                // if(player has only 2 rows of breasts)
                if (player.body.chest.length === 2) {
                    ContentView_1.CView.text("\n\nUnfortunately it looks like you won't find out, as the last of your " + BreastDescriptor_1.describeBreastRow(player.body.chest.get(1)) + " runs dry.  The imp wobbles and falls over, clearly not used to the added weight.  Now that you get a good look at him, you see some very serious changes.  He's got a very full belly, his chest has a pair of soft male breasts, and his cock and balls have shrunk significantly.  It's a damn shame you ran out of milk for the creature.  It would've interesting to see what happened if he'd continued.");
                    ...
                    ContentView_1.CView.text("\n\nYou begin massaging your lowest row of breasts, fascinated by the idea of what will become of the imp when he milks you of all your corrupt milk.  You feel the fluid flow begin, and the imp moves on to your " + BreastDescriptor_1.describeBreastRow(player.body.chest.get(2)) + ".  He nurses passionately at your " + BreastDescriptor_1.describeNipple(player, player.body.chest.get(2)) + ", slurping down every drop of your milk.");
        ...
        player.milked();
        BreastModifier_1.boostLactation(player, 0.1);
        return { next: PassTime_1.passTime(1) };
    }
    exports.feederBreastfeedRape = feederBreastfeedRape;
    // MALE LOSE
    function loseToImpLord(player, impLord) {
        ContentView_1.CView.clear();
        ContentView_1.CView.image("implord-loss-male");
        ContentView_1.CView.text("Unable to control your lust you fall to the ground, remove your " + player.inventory.armor.displayName + " and begin masturbating furiously.  The powerful imp saunters over to you smirking evilly as he towers over your fallen form. You look up at him nervously.  He grabs your chin with one of his clawed hands, while the other digs through his satchel.  He pulls out a vial filled with glowing green liquid, and pops the cork stopper off with his thumb. Before you can react, the demon forces open your mouth and pours the liquid in.  Instinct reacts faster than logic and you swallow the substance as it's poured down your throat.");
        ...
        if (player.body.cocks.length === 1) {
            ContentView_1.CView.text("\n\nYou feel your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + " grow harder than usual and throb.  You go to stroke yourself but it's far too sensitive. Any stroking you can do is far too little stimulation and anything else is too painful to withstand.  You whimper and curse in desperation.  Your lust clouded mind can only think of one solution; you bend over and reveal your [asshole] to the grinning imp.  The humiliation keeps you from looking back to see the imp's reaction, but you can tell by his chuckle that this is exactly what he wanted.");
        }
        else if (player.body.cocks.length > 1) {
            ContentView_1.CView.text("\n\nYou feel your " + CockDescriptor_1.describeCocksLight(player) + " grow harder than usual and throb.  You go to stroke yourself but they are far too sensitive. Any stroking you can do is far too little stimulation and anything else is to painful too withstand.  You whimper and curse in desperation.  Your lust clouded mind can only think of one solution; you bend over and reveal your " + ButtDescriptor_1.describeButthole(player.body.butt) + " to the grinning imp.  The humiliation keeps you from looking back to see the imp's reaction, but you can tell by his chuckle that this is exactly what he wanted.");
        }
        ContentView_1.CView.text("\n\nThe imp gets behind you; his corrupt presence makes the air feel heavy and hard to breathe.  You notice his satchel and loincloth get carelessly tossed to the ground.  Chancing a glance back, you look in aroused horror at the " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " between the imp's legs as well as his matching cum-filled balls.  Two clawed, red hands spread your [butt] revealing your [asshole].  Mercifully, the demon decides you'll need some form of lubrication and relaxation before he continues.  He leans forward and presses his tongue between your [butt] and begins lapping at your [asshole] viciously.  You can't help but mewl from the merciless attack on your tender rectum.");
        ...
        if (player.body.vaginas.length > 0) {
            ...
            ContentView_1.CView.text("\n\nThe imp moves away from your [asshole], and begins to focus on your [balls].  He pulls one into his hand, and squeezes it cruelly while he licks and bites at your [sack].  He gives a painfully tight squeeze to the orb in his hand, which makes you cry out in painful ecstasy.  A single bead of precum gets forced out of your " + CockDescriptor_1.describeCock(player, player.body.cocks.get(0)) + ".");
        }
        ContentView_1.CView.text("\n\nThe imp finally backs off from his brutal attack on your sensitive backside.  Whatever was in that vial has made your body incredibly sensitive... each caress feels like an orgasm, and each scratch feels like a stab wound.  You hope that's the only effect of the green liquid, but don't get much chance to ponder it as you feel the muscular demon press the head of his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " against your [asshole].");
        ContentView_1.CView.text("\n\nYou whimper in fear as you look back towards the devilish imp behind you.  He simply grins at you in response as he thrusts forward.  You yell out in pain as the " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " forces its way into your [asshole].  You try to struggle away, but the imp gives you a very rough slap on the ass.  He then roughly grabs your [hips], making sure to dig his claws in just enough to deter you from struggling.");
        ButtModifier_1.displayStretchButt(player, impLord.body.cocks.get(0).area, true, true, false);
        ...
        ContentView_1.CView.text("\n\nAfter longer than you'd have hoped for, the painful stretching sensation begins to disappear; and the pleasurable sensation of the imp's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " thrusting in and out of your [asshole] becomes entirely pleasurable.  The way his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " fills every inch of your ass, and rubs all your most sensitive spots.  The weird sensation his warm, demonic pre-cum coats your insides.  You find your lust-blinded mind has become lost in the sensations - so lost that you don't even notice the imp increasing his pace.");
        ContentView_1.CView.text("\n\nWithin moments the beast is wildly thrusting in and out of your [asshole].  Pre-cum is pumping out of his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " like a faucet. The hot demon pre begins to spill back out of your abused [asshole], coating your [hips], and dripping to the ground beneath.  The imp gives you a few more rough thrusts before cumming hard into your [asshole].  The little demon's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " spasms as he continues to roughly thrust and pump you full of his burning hot demon seed.");
        if (player.body.cocks.length > 0) {
            ...
            if (player.body.balls.count > 0)
                ContentView_1.CView.text("Your [balls] clench up against your body, desperate to finally expel their contents.  ");
            ContentView_1.CView.text("Your seed spills across the ground, mixing with the copious amount of demon pre that had sloshed to the ground earlier.  You howl loudly in pleasure, as you're finally given release.");
        }
        ContentView_1.CView.text("\n\nThe imp pulls out, but is quick to stuff a soft unknown object into your [asshole] to plug all of his delicious, corrupt seed inside of you.  You stay in position, though you're wobbling slightly from the intense experience.  The short, muscular demon looks down at you, and you look up at him concerned.  He chuckles, \"<i>Don't worry my bitch, that thing will dissolve on its own in a day or so,</i>\" the demon assures you.  He grips his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + ", which is soaked with his own juices, and holds it out towards you.");
    ...
    // FEMALE LOSE
    function getRapedAsAGirl(player, impLord) {
        ContentView_1.CView.clear();
        ContentView_1.CView.image("implord-loss-female");
        ContentView_1.CView.text("You collapse from exhaustion, your [vagina] beginning to soak your [armor].  You groan loudly, desperately trying to continue the fight, or flee, but the exhaustion is too much.  You close your eyes for a moment, but hearing a loud thud near your face causes you to painfully open your eyes.  You see a large bestial hoof near your face, while the other hoof is used to roll you onto your back.");
        ContentView_1.CView.text("\n\nYou try to move, but before you can even begin to squirm a hoof presses hard between your " + BreastDescriptor_1.describeBreastRow(player.body.chest.get(0)) + ".  You gasp as the air is temporarily knocked out of your lungs.  The demon chuckles at your last feeble attempt to free yourself.  He holds his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " stroking it lewdly, a cruel smirk stretching across his face.  You watch as several beads of pre begin to drip from his tip onto your stomach.");
        ...
        if (player.body.legs.isBiped())
            ContentView_1.CView.text(" between your legs");
        else
            ContentView_1.CView.text(" before you");
        ContentView_1.CView.text(", draping his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " across your wet crotch.  You groan, and unintentionally thrust against the magnificent tool between your legs.  The imp chuckles evilly as you coat his " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " in your girl juice, but he doesn't wait long before he slowly presses his head down against your [vagina].  His head slowly spreads your lips; the pleasure is unmistakable, and forces a loud moan from your lips.");
        ...
        VaginaModifier_1.displayStretchVagina(player, impLord.body.cocks.get(0).area, true, true, false);
        ContentView_1.CView.text("\n\nYou mewl softly as you're viciously fucked by the beast above you.  It doesn't take long before your [vagina] clenches tightly around the " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " as you orgasm.  You scream in pleasure as your inner walls begin to milk the imp's " + CockDescriptor_1.describeCockShort(impLord.body.cocks.get(0)) + " of its seed.  The imp quickly succumbs and cums, his swollen balls tightening up against his crotch.  The hot jizz continues to pump into you for what feels like several painfully long minutes, until your belly bulges slightly, and your " + VaginaDescriptor_1.describeVagina(player, player.body.vaginas.get(0)) + " begins to leak the white demonic fluid.");
...
//# sourceMappingURL=ImpLordScene.js.map