define(["require", "exports", "Engine/Items/Consumable", "../ConsumableName", "Engine/Utilities/SMath", "Engine/Body/BreastRow", "Engine/Body/Cock", "Engine/Body/Antennae", "Engine/Body/Horns", "Engine/Body/Legs", "Content/Body/Pregnancy/PregnancyType", "Engine/Body/Skin", "Engine/Body/Tail", "Engine/Body/Wings", "Content/Effects/EffectType", "Engine/Items/ItemDesc", "Content/Descriptors/HairDescriptor", "Content/Descriptors/BreastDescriptor", "Content/Descriptors/ButtDescriptor", "Content/Descriptors/CockDescriptor", "Content/Descriptors/BodyDescriptor", "Engine/Display/ContentView", "Engine/Body/Pregnancy/Womb", "Content/Player/PlayerFlags", "Content/Settings", "Content/Scenes/NPCs/Exgartuan"], function (require, exports, Consumable_1, ConsumableName_1, SMath_1, BreastRow_1, Cock_1, Antennae_1, Horns_1, Legs_1, PregnancyType_1, Skin_1, Tail_1, Wings_1, EffectType_1, ItemDesc_1, HairDescriptor_1, BreastDescriptor_1, ButtDescriptor_1, CockDescriptor_1, BodyDescriptor_1, ContentView_1, Womb_1, PlayerFlags_1, Settings_1, Exgartuan_1) {
    "use strict";
    ...
    class BeeHoney extends Consumable_1.Consumable {
        constructor(pure, special) {
            ...
            let honeyValue;
            if (special) {
                honeyName = ConsumableName_1.ConsumableName.BeeHoneySpecial;
                ...
                honeyName = (pure ? ConsumableName_1.ConsumableName.BeeHoneyPure : ConsumableName_1.ConsumableName.BeeHoney);
            ...
            super(honeyName, new ItemDesc_1.ItemDesc(honeyName, honeyLong, honeyDesc), honeyValue);
        }
        canUse(character) {
            if (this.value === BeeHoney.SPECIAL_HONEY_VALUE && Exgartuan_1.ExgartuanFlags.LOCATION === 1) { // Exgartuan doesn't like the special honey
                ContentView_1.CView.text("You uncork the bottle only to hear Exgartuan suddenly speak up.  <i>“Hey kid, this beautiful cock here doesn’t need any of that special bee shit.  Cork that bottle up right now or I’m going to make it so that you can’t drink anything but me.”</i>  You give an exasperated sigh and put the cork back in the bottle.");
                return false;
            }
            return true;
        }
        isPregnantWithFaerie(character) {
            return (character.body.wombs.find(Womb_1.Womb.Pregnant) && !!character.body.wombs.find(Womb_1.Womb.PregnantWithType(PregnancyType_1.PregnancyType.FAERIE))) ||
                (character.body.buttWomb.isPregnant() && character.body.buttWomb.pregnancy.type === PregnancyType_1.PregnancyType.FAERIE);
        }
        use(character) {
            const pure = (this.value === BeeHoney.PURE_HONEY_VALUE);
            const special = (this.value === BeeHoney.SPECIAL_HONEY_VALUE);
            let changes = 0;
            let changeLimit = 1;
            ...
            // Drink text
            if (special) {
                ContentView_1.CView.text("You uncork the bottle and pour the incredibly strong smelling concentrated honey down your throat.  Its taste is also mighty intense.  All at once you feel the effects of the substance start to course through your body.");
            }
            else { // Text for normal or pure
                ContentView_1.CView.text("Opening the crystal vial, you are greeted by a super-concentrated wave of sweet honey-scent.  It makes you feel lightheaded.  You giggle and lick the honey from your lips, having drank down the syrupy elixir without a thought.");
            }
            if ((pure || special) && this.isPregnantWithFaerie(character)) { // Pure or special honey can reduce the corruption of a phouka baby
                if (PlayerFlags_1.PlayerFlags.PREGNANCY_CORRUPTION > 1) { // Child is phouka, hates pure honey
                ...
                else if (PlayerFlags_1.PlayerFlags.PREGNANCY_CORRUPTION < 1) { // Child is faerie, loves pure honey
                ...
                PlayerFlags_1.PlayerFlags.PREGNANCY_CORRUPTION--;
                if (pure)
            ...
            if (changes < changeLimit && pure) { // Special honey will also reduce corruption, but uses different text and is handled separately
                ...
                if (character.stats.cor > 80)
                    ContentView_1.CView.text("Your head aches, as if thunder was echoing around your skull.  ");
                else if (character.stats.cor > 60)
                    ContentView_1.CView.text("You feel a headache forming just behind your eyes.  In no time flat it reaches full strength.  ");
                else if (character.stats.cor > 40)
                    ContentView_1.CView.text("A wave of stinging pain slices through your skull.  ");
                else if (character.stats.cor > 20)
                    ContentView_1.CView.text("A prickling pain spreads throughout your skull.  ");
                else
                    ContentView_1.CView.text("You feel a mildly unpleasant tingling inside your skull.  ");
                if (character.stats.cor > 0)
                    ContentView_1.CView.text("It quickly passes, leaving you more clearheaded");
                character.stats.cor += -(1 + (character.stats.cor / 20));
                // Libido Reduction
                if (character.stats.cor > 0 && changes < changeLimit && SMath_1.randInt(3) === 0 && character.stats.lib > 40) {
                    ContentView_1.CView.text(" and settling your overcharged sex-drive a bit.");
                    character.stats.lib -= 3;
                    character.stats.lust -= 20;
                    changes++;
                }
                else if (character.stats.cor > 0)
                    ContentView_1.CView.text(".");
            }
            // bee item corollary:
            ...
            // Intelligence Boost
            if (changes < changeLimit && SMath_1.randInt(2) === 0 && character.stats.int < 80) {
            ...
            // Appearance Stuff
            // Hair Color
            if (changes < changeLimit && (character.body.hair.color !== "shiny black" && character.body.hair.color !== "black and yellow") && character.body.hair.length > 10 && SMath_1.randInt(2) === 0) {
                ContentView_1.CView.text("\n\nYou feel your scalp tingling, and you grab your hair in a panic, pulling a strand forward.  ");
                if (SMath_1.randInt(9) === 0)
                    character.body.hair.color = "black and yellow";
                else
                    character.body.hair.color = "shiny black";
                ContentView_1.CView.text("Your hair is now " + character.body.hair.color + ", just like a bee-girl's!");
                changes++;
            }
            // Hair Length
            if (changes < changeLimit && character.body.hair.length < 25 && SMath_1.randInt(3) === 0) {
                ContentView_1.CView.text("\n\nFeeling a bit off-balance, you discover your hair has lengthened, ");
                character.body.hair.length += SMath_1.randInt(4) + 1;
                ContentView_1.CView.text("becoming " + HairDescriptor_1.describeHair(character) + ".");
                changes++;
            }
            // -Remove extra breast rows
            if (changes < changeLimit && chest.length > 2 && SMath_1.randInt(3) === 0 && !Settings_1.Settings.hyperHappy) {
                changes++;
            ...
            // Antennae
            if (changes < changeLimit && character.body.antennae.type === Antennae_1.AntennaeType.NONE && character.body.horns.count === 0 && SMath_1.randInt(3) === 0) {
                ContentView_1.CView.text("\n\nYour head itches momentarily as two floppy antennae sprout from your " + HairDescriptor_1.describeHair(character) + ".");
                character.body.antennae.type = Antennae_1.AntennaeType.BEE;
                changes++;
            }
            // Horns
            if (changes < changeLimit && character.body.horns.count > 0 && SMath_1.randInt(3) === 0) {
            ...
            // Bee Legs
            if (changes < changeLimit && character.body.legs.type !== Legs_1.LegType.BEE && character.body.legs.type !== Legs_1.LegType.CENTAUR && SMath_1.randInt(4) === 0) {
                ...
                character.body.legs.type = Legs_1.LegType.BEE;
                changes++;
            }
            // -Nipples reduction to 1 per tit.
            if (chest.reduce(BreastRow_1.BreastRow.AverageNipplesPerBreast, 0) > 1 && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            // Gain oviposition!
            if (changes < changeLimit && !character.effects.has(EffectType_1.EffectType.BeeOvipositor) && character.body.tails.filter(Tail_1.Tail.FilterType(Tail_1.TailType.BEE_ABDOMEN)).length > 1 && SMath_1.randInt(2) === 0) {
            ...
            if (changes < changeLimit && character.body.wings.type === Wings_1.WingType.BEE_LIKE_SMALL && SMath_1.randInt(4)) {
                changes++;
                character.body.wings.type = Wings_1.WingType.BEE_LIKE_LARGE;
            ...
            if (changes < changeLimit && (character.body.wings.type === Wings_1.WingType.NONE || character.body.wings.type === Wings_1.WingType.SHARK_FIN) && SMath_1.randInt(4)) {
                changes++;
                ...
                character.body.wings.type = Wings_1.WingType.BEE_LIKE_SMALL;
                character.body.wings.desc = "small bee-like";
            }
            // Melt demon wings!
            if (changes < changeLimit && (character.body.wings.type === Wings_1.WingType.BAT_LIKE_TINY || character.body.wings.type === Wings_1.WingType.BAT_LIKE_LARGE)) {
                changes++;
            ...
            if (special) { // All the speical honey effects occur after any normal bee transformations (if the character wasn't a full bee morph)
                ...
                if (cocks.length <= 0) {
                    ...
                    selectedCock.length = SMath_1.randInt(3) + 8;
                    selectedCock.thickness = 2;
                    cocks.add(selectedCock);
                    character.stats.sens += 10;
                }
                else if (cocks.length > 1) {
                    const largestCock = cocks.sort(Cock_1.Cock.Largest).get(0);
                    selectedCock = cocks.get(0);
                    ContentView_1.CView.text("\n\nThe effects of the honey move towards your groin, and into your " + CockDescriptor_1.describeCocksLight(character) + ", causing them to stand at attention.  They quiver for a moment, and feel rather itchy.  Suddenly you are overwhelmed with pleasure as <b>your " + CockDescriptor_1.describeCock(character, largestCock) + " is absorbed into your " + CockDescriptor_1.describeCock(character, selectedCock) + "!</b>  You grab onto the merging cock and pump it with your hands as it increases in size and you cum in pleasure.  Your " + CockDescriptor_1.describeCock(character, selectedCock) + " seems a lot more sensative now...");
                    ...
                    selectedCock = cocks.get(0);
                    ContentView_1.CView.text("\n\nYour " + CockDescriptor_1.describeCock(character, selectedCock) + " suddenly becomes rock hard and incredibly sensitive to the touch.  You pull away your " + character.inventory.armor.displayName + ", and start to masturbate furiously as it rapidly swells in size.  When the change finally finishes, you realize that your " + CockDescriptor_1.describeCock(character, selectedCock) + " has both grown much longer and wider!  <b>");
                    if (selectedCock.area <= 20)
                        ContentView_1.CView.text("It now swings as low as your knees!");
                    else if (selectedCock.area <= 50)
                        ContentView_1.CView.text("While erect, your massive member fills the lower half of your vision.");
                    else
                        ContentView_1.CView.text("Your member is now simply huge, you wonder what in the world could actually take your massive size now?");
                    ContentView_1.CView.text("</b>");
                    selectedCock.length += SMath_1.randInt(3) + 4; // 4 to 6 inches in length
                    selectedCock.thickness += 0.1 * SMath_1.randInt(5) + 0.5; // 0.5 to 1 inches in thickness
                    character.stats.sens += 5;
                }
                else if (cocks.get(0).type !== Cock_1.CockType.BEE && BodyDescriptor_1.describeRace(character) === "bee-morph") {
                    selectedCock = cocks.get(0);
                    ContentView_1.CView.text("\n\nYour huge member suddenly starts to hurt, especially the tip of the thing.  At the same time, you feel your length start to get incredibly sensitive and the base of your shaft starts to itch.  You tear off your " + character.inventory.armor.displayName + " and watch in fascination as your " + CockDescriptor_1.describeCock(character, selectedCock) + " starts to change.  The shaft turns black, while becoming hard and smooth to the touch, while the base develops a mane of four inch long yellow bee hair.  As the transformation continues, your member grows even larger than before.  However, it is the tip that keeps your attention the most, as a much finer layer of short yellow hairs grow around it.  Its appearance isn’t the thing that you care about right now, it is the pain that is filling it.\n\n");
                    ContentView_1.CView.text("It is entirely different from the usual feeling you get when you’re cock grows larger from imbibing transformative substances.  When the changes stop, the tip is shaped like a typical human mushroom cap covered in fine bee hair, but it feels nothing like what you’d expect a human dick to feel like.  Your whole length is incredibly sensitive, and touching it gives you incredible stimulation, but you’re sure that no matter how much you rub it, you aren’t going to cum by yourself.  You want cool honey covering it, you want tight walls surrounding it, you want to fertilize hundreds of eggs with it.  These desires are almost overwhelming, and it takes a lot of will not to just run off in search of the bee girl that gave you that special honey right now.  This isn’t good.\n\n");
                    ContentView_1.CView.text("<b>You now have a bee cock!</b>");
                    selectedCock.type = Cock_1.CockType.BEE;
                    ...
                    selectedCock = cocks.get(0);
                    ContentView_1.CView.text("\n\nThe effects of the honey don’t seem to focus on your groin this time, but you still feel your " + CockDescriptor_1.describeCock(character, selectedCock) + " grow slightly under your " + character.inventory.armor.displayName + ".");
                    selectedCock.length += 0.1 * SMath_1.randInt(10) + 1;
                    selectedCock.thickness += 0.1 * SMath_1.randInt(2) + 0.1;
                ...
                if (character.stats.cor >= 5) {
                    ...
                    const corLoss = Math.min(0.1 * character.stats.cor + 5, character.stats.cor);
