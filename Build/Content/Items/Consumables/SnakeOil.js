define(["require", "exports", "Engine/Items/Consumable", "../ConsumableName", "Engine/Utilities/SMath", "Engine/Body/Face", "Engine/Body/Antennae", "Engine/Body/Legs", "Engine/Body/Tongue", "Engine/Body/Wings", "Content/Effects/EffectType", "Engine/Items/ItemDesc", "Content/Descriptors/FaceDescriptor", "Content/Descriptors/LegDescriptor", "Engine/Display/ContentView"], function (require, exports, Consumable_1, ConsumableName_1, SMath_1, Face_1, Antennae_1, Legs_1, Tongue_1, Wings_1, EffectType_1, ItemDesc_1, FaceDescriptor_1, LegDescriptor_1, ContentView_1) {
    "use strict";
    ...
    /*Effects:
        ...
        */
    class SnakeOil extends Consumable_1.Consumable {
        constructor() {
            super(ConsumableName_1.ConsumableName.SnakeOil, new ItemDesc_1.ItemDesc("SnakOil", "a vial of snake oil", "A vial the size of your fist made of dark brown glass. It contains what appears to be an oily, yellowish liquid. The odor is abominable."));
        }
        use(character) {
            ...
            ContentView_1.CView.text("Pinching your nose, you quickly uncork the vial and bring it to your mouth, determined to see what effects it might have on your body. Pouring in as much as you can take, you painfully swallow before going for another shot, emptying the bottle.");
            // (if outside combat)
            // if (!Game.inCombat) CView.text("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.");
            // + speed to 70!
            if (character.stats.spe < 70 && SMath_1.randInt(2) === 0) {
                ...
                if (character.stats.spe < 40)
                    ContentView_1.CView.text("  Of course, you're nowhere near as fast as that.");
                changes++;
            }
            // Removes wings
            if (character.body.wings.type > Wings_1.WingType.NONE && SMath_1.randInt(3) === 0 && changes < changeLimit) {
            ...
            // Removes antennae
            if (character.body.antennae.type > Antennae_1.AntennaeType.NONE && SMath_1.randInt(3) === 0 && changes < changeLimit) {
            ...
            if (changes === 0 && character.body.tongue.type !== Tongue_1.TongueType.SNAKE && SMath_1.randInt(3) === 0 && changes < changeLimit) {
                if (character.body.tongue.type === Tongue_1.TongueType.HUMAN)
                ...
                character.body.tongue.type = Tongue_1.TongueType.SNAKE;
                character.stats.sens += 5;
                changes++;
            }
            // 9c) III The fangs
            if (changes === 0 && character.body.tongue.type === Tongue_1.TongueType.SNAKE && character.body.face.type !== Face_1.FaceType.SNAKE_FANGS && SMath_1.randInt(3) === 0 && changes < changeLimit) {
                ...
                if (character.body.face.type !== Face_1.FaceType.HUMAN && character.body.face.type !== Face_1.FaceType.SHARK_TEETH && character.body.face.type !== Face_1.FaceType.BUNNY && character.body.face.type !== Face_1.FaceType.SPIDER_FANGS) {
                    ContentView_1.CView.text("  As the change progresses, your " + FaceDescriptor_1.describeFaceShort(character) + " reshapes.  The sensation is far more pleasant than teeth cutting into gums, and as the tingling transformation completes, <b>you've gained with a normal-looking, human visage.</b>");
                }
                character.body.face.type = Face_1.FaceType.SNAKE_FANGS;
                changes++;
            }
            ...
            if (changes === 0 && character.body.face.type === Face_1.FaceType.SNAKE_FANGS && character.body.legs.type !== Legs_1.LegType.NAGA && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                ContentView_1.CView.text("  The process continues through the length of your " + LegDescriptor_1.describeLegs(character) + ", eventually reaching your " + LegDescriptor_1.describeFeet(character) + ".  Just when you think that the transformation is over, you find yourself pinned to the ground by an overwhelming sensation of pain. You hear the horrible sound of your bones snapping, fusing together and changing into something else while you contort in unthinkable agony.  Sometime later you feel the pain begin to ease and you lay on the ground, spent by the terrible experience. Once you feel you've recovered, you try to stand, but to your amazement you discover that you no longer have " + LegDescriptor_1.describeLegs(character) + ": the bottom half of your body is like that of a snake's.");
                ...
                if (character.body.balls.count > 0 && character.body.balls.size > 10)
                    ContentView_1.CView.text("  You're happy not to have to drag those testicles around with you anymore.");
                ContentView_1.CView.text("  But then, scales start to form on the surface of your skin, slowly becoming visible, recoloring all of your body from the waist down in a snake-like pattern. The feeling is... not that bad actually, kind of like callous, except on your whole lower body. The transformation complete, you get up, standing on your newly formed snake tail. You can't help feeling proud of this majestic new body of yours.");
                character.body.legs.type = Legs_1.LegType.NAGA;
                changes++;
            }
    ...
    exports.SnakeOil = SnakeOil;
});
//# sourceMappingURL=SnakeOil.js.map