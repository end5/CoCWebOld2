define(["require", "exports", "Engine/Items/Consumable", "../ConsumableName", "Engine/Utilities/SMath", "Engine/Body/Arms", "Engine/Body/Cock", "Engine/Body/Ears", "Engine/Body/Eyes", "Engine/Body/Face", "Engine/Body/Legs", "Engine/Body/Skin", "Engine/Body/Tail", "Content/Effects/EffectType", "Engine/Items/ItemDesc", "Content/Descriptors/LegDescriptor", "Content/Descriptors/BallsDescriptor", "Content/Descriptors/VaginaDescriptor", "Content/Descriptors/CockDescriptor", "Engine/Display/ContentView", "Content/Body/RaceScore", "Content/Menus/InGame/GameOverMenu"], function (require, exports, Consumable_1, ConsumableName_1, SMath_1, Arms_1, Cock_1, Ears_1, Eyes_1, Face_1, Legs_1, Skin_1, Tail_1, EffectType_1, ItemDesc_1, LegDescriptor_1, BallsDescriptor_1, VaginaDescriptor_1, CockDescriptor_1, ContentView_1, RaceScore_1, GameOverMenu_1) {
    "use strict";
    ...
    /*
     ...
     -Fur
     -Roo face*/
    class KangaFruit extends Consumable_1.Consumable {
        constructor(enhanced) {
            if (enhanced)
                super(ConsumableName_1.ConsumableName.KangaFruitEnhanced, new ItemDesc_1.ItemDesc("MghtyVg", "a mightily enhanced piece of kanga fruit", "A yellow, fibrous, tubular pod.  A split in the end reveals many lumpy, small seeds inside.  The smell of mild fermentation wafts from them.  It glows slightly from Lumi's enhancements."));
            else
                super(ConsumableName_1.ConsumableName.KangaFruit, new ItemDesc_1.ItemDesc("KangaFruit", "a piece of kanga fruit", "A yellow, fibrous, tubular pod.  A split in the end reveals many lumpy, small seeds inside.  The smell of mild fermentation wafts from them."));
            this.enhanced = enhanced;
        }
        use(character) {
            ContentView_1.CView.clear();
            ContentView_1.CView.text("You squeeze the pod around the middle, forcing the end open.  Scooping out a handful of the yeasty-smelling seeds, you shovel them in your mouth.  Blech!  Tastes like soggy burnt bread... and yet, you find yourself going for another handful...");
            ...
            if (character.stats.int < 10) {
                if (character.stats.int < 8 && RaceScore_1.kangaRaceScore(character) >= 5) {
            ...
            // -Speed to 70
            if (character.stats.spe < 70 && SMath_1.randInt(3) === 0) {
            ...
            // -Int to 10
            if (character.stats.int > 2 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
                changes++;
                // Gain dumb (smart!)
                ...
                // gain dumb (30-10 int):
                else if (character.stats.int > 10)
            ...
            // ****************
            // -Hip widening funtimes
            if (changes < changeLimit && SMath_1.randInt(4) === 0 && character.body.hips.rating < 40) {
            ...
            // ****************
            // -Shrink balls down to reasonable size (3?)
            if (character.body.balls.size >= 4 && changes < changeLimit && SMath_1.randInt(2) === 0) {
                character.body.balls.size--;
                character.body.cumMultiplier++;
                ContentView_1.CView.text("\n\nYour " + BallsDescriptor_1.describeSack(character) + " pulls tight against your groin, vibrating slightly as it changes.  Once it finishes, you give your " + BallsDescriptor_1.describeBallsShort(character) + " a gentle squeeze and discover they've shrunk.  Even with the reduced volume, they feel just as heavy.");
                changes++;
            }
            // -Shorten clits to reasonable size
            if (character.body.clit.length >= 4 && changes < changeLimit && SMath_1.randInt(5) === 0) {
                ContentView_1.CView.text("\n\nPainful pricks work through your " + VaginaDescriptor_1.describeClit(character) + ", all the way into its swollen clitoral sheath.  Gods, it feels afire with pain!  Agony runs up and down its length, and by the time the pain finally fades, the feminine organ has lost half its size.");
            ...
            if (character.body.cocks.length > 0) {
                if (biggestCock.length >= 16 && changes < changeLimit && SMath_1.randInt(5) === 0) {
                    ContentView_1.CView.text("\n\nA roiling inferno of heat blazes in your " + CockDescriptor_1.describeCock(character, biggestCock) + ", doubling you over in the dirt.  You rock back and forth while tears run unchecked down your cheeks.  Once the pain subsides and you're able to move again, you find the poor member has lost nearly half its size.");
                    biggestCock.length /= 2;
                    biggestCock.thickness /= 1.5;
                    if (biggestCock.thickness * 6 > biggestCock.length)
                        biggestCock.thickness -= .2;
                    if (biggestCock.thickness * 8 > biggestCock.length)
                        biggestCock.thickness -= .2;
                    if (biggestCock.thickness < .5)
                        biggestCock.thickness = .5;
                    changes++;
                }
                // COCK TF!
                if (character.body.cocks.filter(Cock_1.Cock.FilterType(Cock_1.CockType.KANGAROO)).length < character.body.cocks.length && (this.enhanced && SMath_1.randInt(2) === 0) && changes < changeLimit) {
                    ...
                    if (character.body.cocks.find(Cock_1.Cock.HasSheath))
                        ContentView_1.CView.text("your sheath");
                    else
                        ContentView_1.CView.text("a sheath that forms at the base of it");
                    ContentView_1.CView.text(".  <b>You now have a kangaroo-penis!</b>");
                    // Find first non-roocock!
                    for (const cock of character.body.cocks) {
                        if (cock.type !== Cock_1.CockType.KANGAROO) {
                            cock.type = Cock_1.CockType.KANGAROO;
            ...
            // ****************
            // -Face (Req: Fur + Feet)
            if (character.body.face.type !== Face_1.FaceType.KANGAROO && ((character.body.skin.type === Skin_1.SkinType.FUR && character.body.legs.type === Legs_1.LegType.KANGAROO) || this.enhanced) && changes < changeLimit && SMath_1.randInt(4) === 0) {
                // gain roo face from human/naga/shark/bun:
                if (character.body.face.type === Face_1.FaceType.HUMAN || character.body.face.type === Face_1.FaceType.SNAKE_FANGS || character.body.face.type === Face_1.FaceType.SHARK_TEETH || character.body.face.type === Face_1.FaceType.BUNNY)
                ...
                changes++;
                character.body.face.type = Face_1.FaceType.KANGAROO;
            }
            // -Fur (Req: Footsies)
            if (character.body.skin.type !== Skin_1.SkinType.FUR && (character.body.legs.type === Legs_1.LegType.KANGAROO || this.enhanced) && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            if (character.body.legs.type !== Legs_1.LegType.KANGAROO && (this.enhanced || character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.KANGAROO), false)) && changes < changeLimit && SMath_1.randInt(4) === 0) {
                ...
                else if (character.body.legs.type === Legs_1.LegType.NAGA)
                ...
                character.body.legs.type = Legs_1.LegType.KANGAROO;
                changes++;
            }
            // -Roo tail (Req: Ears)
            if (!character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.KANGAROO), false) && changes < changeLimit && SMath_1.randInt(4) === 0 && (!this.enhanced || character.body.ears.type === Ears_1.EarType.KANGAROO)) {
                // gain roo tail:
                ...
                // gain roo tail from bee tail:
                else if (character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.SPIDER_ABDOMEN), false) || character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.BEE_ABDOMEN), false)) {
                    ...
                    if (character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.BEE_ABDOMEN), false))
                        ContentView_1.CView.text("hollow stinger drops out of the end, taking the venom organ with it.");
                    else
                        ContentView_1.CView.text("spinnerets drop out of the end, taking the last of your webbing with it.");
                    ContentView_1.CView.text("  By the time you're back to yourself, the insectile carapace has fallen off completely, leaving you with a long, thick, fleshy tail in place of your proud, insectile abdomen.  <b>You now have a kangaroo tail!</b>  You wipe the errant spittle from your mouth as you idly bob your new tail about.");
                }
                ...
                newTail.type = Tail_1.TailType.KANGAROO;
                character.body.tails.add(newTail);
                changes++;
            }
            // -Roo ears
            if (character.body.ears.type !== Ears_1.EarType.KANGAROO && changes < changeLimit && SMath_1.randInt(4) === 0) {
                ...
                changes++;
                character.body.ears.type = Ears_1.EarType.KANGAROO;
            }
            // UBEROOOO
            // kangaroo perk: - any liquid or food intake will accelerate a pregnancy, but it will not progress otherwise
            if (!character.effects.has(EffectType_1.EffectType.Diapause) && RaceScore_1.kangaRaceScore(character) > 4 && SMath_1.randInt(4) === 0 && changes < changeLimit && character.body.vaginas.length > 0) {
                // Perk name and description:
                character.effects.create(EffectType_1.EffectType.Diapause);
    ...
    exports.KangaFruit = KangaFruit;
});
//# sourceMappingURL=KangaFruit.js.map