define(["require", "exports", "Engine/Items/Consumable", "../ConsumableName", "Engine/Utilities/SMath", "Engine/Body/Ears", "Engine/Body/Face", "Engine/Body/Legs", "Engine/Body/Skin", "Engine/Body/Tail", "Engine/Items/ItemDesc", "Content/Descriptors/SkinDescriptor", "Content/Descriptors/HairDescriptor", "Engine/Display/ContentView", "Content/Modifiers/BodyModifier"], function (require, exports, Consumable_1, ConsumableName_1, SMath_1, Ears_1, Face_1, Legs_1, Skin_1, Tail_1, ItemDesc_1, SkinDescriptor_1, HairDescriptor_1, ContentView_1, BodyModifier_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class RingtailFig extends Consumable_1.Consumable {
        constructor() {
            super(ConsumableName_1.ConsumableName.RingtailFig, new ItemDesc_1.ItemDesc("RingFig", "a ringtail fig", "A dried fig with two lobes and thin dark rings just below its stem.  The skin is wrinkly and it looks vaguely like a bulging scrotum."));
        }
        use(character) {
            ContentView_1.CView.clear();
            // eat it:
            ContentView_1.CView.text("You split the fruit and scoop out the pulp, eating it greedily.  It's sweet and slightly gritty with seeds, and you quickly finish both halves.");
            let changes = 0;
            let changeLimit = 1;
            ...
            // gain sensitivity
            if (character.stats.sens < 80 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
                ContentView_1.CView.text("\n\nThe wrinkled rind suddenly feels alarmingly distinct in your hands, and you drop the remnants of the fruit.  Wonderingly, you touch yourself with a finger - you can feel even the lightest pressure on your " + SkinDescriptor_1.skinFurScales(character) + " much more clearly now!");
                if (character.stats.sens < 60)
                    character.stats.sens += 2;
                character.stats.sens += 2;
                changes++;
            }
            // lose toughness to floor of 50
            if (SMath_1.randInt(4) && character.stats.tou > 50 && changes < changeLimit) {
            ...
            if (character.body.cocks.length > 0) {
                // gain ball size
                if (character.body.balls.count > 0 && character.body.balls.size < 15 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                if (character.body.balls.count === 0 && character.body.skin.type === Skin_1.SkinType.FUR && SMath_1.randInt(3) === 0 && changes < changeLimit) {
            ...
            if (SMath_1.randInt(4) === 0 && character.body.thickness < 80 && changes < changeLimit) {
                ContentView_1.CView.text(BodyModifier_1.displayModThickness(character, 80, 2));
                changes++;
            }
            // bodypart changes:
            if (character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.RACCOON), false) && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                // grow da tail
                // from no tail:
                ...
                character.body.tails.add(new Tail_1.Tail(Tail_1.TailType.RACCOON));
                changes++;
            }
            // gain fur
            if ((character.body.legs.type === Legs_1.LegType.RACCOON && character.body.ears.type === Ears_1.EarType.RACCOON) && character.body.skin.type !== Skin_1.SkinType.FUR && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            // gain coon ears
            if (character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.RACCOON), false) && character.body.ears.type !== Ears_1.EarType.RACCOON && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                // from dog, kangaroo, bunny, other long ears
                if (character.body.ears.type === Ears_1.EarType.DOG || character.body.ears.type === Ears_1.EarType.BUNNY || character.body.ears.type === Ears_1.EarType.KANGAROO)
                ...
                else if (character.body.ears.type === Ears_1.EarType.HORSE || character.body.ears.type === Ears_1.EarType.COW || character.body.ears.type === Ears_1.EarType.CAT)
                ...
                // from human, goblin, lizard or other short ears
                    ...
                    ContentView_1.CView.text("\n\nYour ears prick and stretch uncomfortably, poking up through your " + HairDescriptor_1.describeHair(character) + ".  Covering them with your hands, you feel them shaping into little eggdrop ornaments resting atop your head.  <b>You have raccoon ears!</b>");
                character.body.ears.type = Ears_1.EarType.RACCOON;
                changes++;
            }
            // gain feet-coon
            if (character.body.ears.type === Ears_1.EarType.RACCOON && character.body.legs.type !== Legs_1.LegType.RACCOON && changes < changeLimit && SMath_1.randInt(4) === 0) {
                ...
                // from hooves or hard feet, including centaurs and bees
                else if (character.body.legs.type === Legs_1.LegType.HOOFED || character.body.legs.type === Legs_1.LegType.CENTAUR || character.body.legs.type === Legs_1.LegType.BEE || character.body.legs.type === Legs_1.LegType.PONY || character.body.legs.type === Legs_1.LegType.CHITINOUS_SPIDER_LEGS) {
                    ...
                    if (character.body.legs.type === Legs_1.LegType.HOOFED || character.body.legs.type === Legs_1.LegType.CENTAUR || character.body.legs.type === Legs_1.LegType.PONY)
                        ContentView_1.CView.text("pops right out of its hoof just in time");
                    else
                        ContentView_1.CView.text("comes loose inside its long boot, and you pull it free with irritation only");
                    ContentView_1.CView.text(" for you to set it back down on a sharp rock!  Biting off a curse, you examine the new bare foot.  It looks much like a human's, except for the nearly-twice-as-long toes.  You find you can even use them to pick things up; the sharp rock is dropped into your hand and tossed far away.  The shed [foot] is quickly joined on the ground by its complement, revealing more long toes.  ");
                    ...
                    ContentView_1.CView.text("<b>You now have two fuzzy, long-toed raccoon legs.</b>");
                }
                // from human, demon, paw feet
                    ...
                    if ((character.body.legs.type === Legs_1.LegType.HUMAN || character.body.legs.type === Legs_1.LegType.DEMONIC_HIGH_HEELS || character.body.legs.type === Legs_1.LegType.DEMONIC_CLAWS) && character.body.skin.type !== Skin_1.SkinType.FUR)
                        ContentView_1.CView.text("The sensation of walking around on what feels like a second pair of hands is so weird that you miss noticing the itchy fur growing in over your legs...  ");
                    ContentView_1.CView.text("<b>You now have raccoon paws!</b>");
                }
                character.body.legs.type = Legs_1.LegType.RACCOON;
                changes++;
            }
            // gain half-coon face (prevented if already full-coon)
            if (character.body.face.type !== Face_1.FaceType.RACCOON_MASK && character.body.face.type !== Face_1.FaceType.RACCOON && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                // from human/naga/shark/bun face
                if (character.body.face.type === Face_1.FaceType.HUMAN || character.body.face.type === Face_1.FaceType.SHARK_TEETH || character.body.face.type === Face_1.FaceType.SNAKE_FANGS || character.body.face.type === Face_1.FaceType.BUNNY) {
                    ContentView_1.CView.text("\n\nA sudden wave of exhaustion passes over you, and your face goes partially numb around your eyes.  ");
                    // (nagasharkbunnies)
                    if (character.body.face.type === Face_1.FaceType.SHARK_TEETH || character.body.face.type === Face_1.FaceType.SNAKE_FANGS || character.body.face.type === Face_1.FaceType.BUNNY) {
                        ContentView_1.CView.text("Your prominent teeth chatter noisily at first, then with diminishing violence, until you can no longer feel them jutting past the rest!  ");
                    }
                    ContentView_1.CView.text("Shaking your head a bit, you wait for your energy to return, then examine your appearance.  ");
                    // (if character skin.tone = ebony/black/ebony with tats and no fur/scales or if black/midnight fur or if black scales
                    if (((character.body.skin.tone === "ebony" || character.body.skin.tone === "black") && (character.body.skin.type === Skin_1.SkinType.PLAIN || character.body.skin.type === Skin_1.SkinType.GOO)) || ((character.body.hair.color === "black" || character.body.hair.color === "midnight") && (character.body.skin.type === Skin_1.SkinType.FUR || character.body.skin.type === Skin_1.SkinType.SCALES))) {
                        ContentView_1.CView.text("Nothing seems different at first.  Strange... you look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask.</b>");
                    }
                    else
                        ContentView_1.CView.text("A dark, almost black mask shades the " + SkinDescriptor_1.skinFurScales(character) + " around your eyes and over the topmost portion of your nose, lending you a criminal air!  <b>You now have a raccoon mask!</b>");
                }
                // from snout (will not overwrite full-coon snout but will overwrite others)
                    ...
                    if (((character.body.hair.color === "black" || character.body.hair.color === "midnight") && (character.body.skin.type === Skin_1.SkinType.FUR || character.body.skin.type === Skin_1.SkinType.SCALES)))
                        ContentView_1.CView.text("your face apparently returned to normal shape, albeit still covered in " + SkinDescriptor_1.skinFurScales(character) + ".  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your otherwise normal human face.</b>");
                    else if ((character.body.skin.tone === "ebony" || character.body.skin.tone === "black") && (character.body.skin.type === Skin_1.SkinType.PLAIN || character.body.skin.type === Skin_1.SkinType.GOO))
                        ContentView_1.CView.text("your face apparently returned to normal shape.  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your normal human face.</b>");
                    else
                        ContentView_1.CView.text("your face returned to human dimensions, but shaded by a black mask around the eyes and over the nose!  <b>You now have a humanoid face with a raccoon mask!</b>");
                }
            ...
            // from humanoid - should be the only one possible
            else if (character.body.face.type === Face_1.FaceType.RACCOON_MASK && character.body.legs.type === Legs_1.LegType.RACCOON && character.body.skin.type === Skin_1.SkinType.FUR && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                changes++;
                character.body.face.type = Face_1.FaceType.RACCOON;
            }
            // fatigue damage (only if face change was not triggered)
            else if (SMath_1.randInt(2) === 0 && changes < changeLimit && (character.body.face.type !== Face_1.FaceType.RACCOON_MASK && character.body.face.type !== Face_1.FaceType.RACCOON)) {
    ...
    exports.RingtailFig = RingtailFig;
});
//# sourceMappingURL=RingtailFig.js.map