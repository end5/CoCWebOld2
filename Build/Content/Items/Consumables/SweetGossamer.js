define(["require", "exports", "Engine/Items/Consumable", "../ConsumableName", "Engine/Utilities/SMath", "Engine/Body/Arms", "Engine/Body/BreastRow", "Engine/Body/Ears", "Engine/Body/Eyes", "Engine/Body/Face", "Engine/Body/Legs", "Engine/Body/Skin", "Engine/Body/Tail", "Content/Effects/EffectType", "Engine/Items/ItemDesc", "Content/Descriptors/SkinDescriptor", "Content/Descriptors/VaginaDescriptor", "Content/Descriptors/ButtDescriptor", "Content/Descriptors/CockDescriptor", "Content/Descriptors/BreastDescriptor", "Content/Descriptors/LegDescriptor", "Engine/Display/ContentView", "Content/Body/RaceScore", "Content/Settings"], function (require, exports, Consumable_1, ConsumableName_1, SMath_1, Arms_1, BreastRow_1, Ears_1, Eyes_1, Face_1, Legs_1, Skin_1, Tail_1, EffectType_1, ItemDesc_1, SkinDescriptor_1, VaginaDescriptor_1, ButtDescriptor_1, CockDescriptor_1, BreastDescriptor_1, LegDescriptor_1, ContentView_1, RaceScore_1, Settings_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class SweetGossamer extends Consumable_1.Consumable {
        constructor(sweet) {
            if (!sweet)
                super(ConsumableName_1.ConsumableName.BlackGossamer, new ItemDesc_1.ItemDesc("B.Gossr", "a bundle of black, gossamer webbing", "These strand(s of gooey black gossamer seem quite unlike the normal silk that driders produce.  It smells sweet and is clearly edible, but who knows what it might do to you?"));
            else
                super(ConsumableName_1.ConsumableName.SweetGossamer, new ItemDesc_1.ItemDesc("S.Gossr", "a bundle of pink, gossamer webbing", "These strand(s of gooey pink gossamer seem quite unlike the normal silk that spider-morphs produce.  It smells sweet and is clearly edible, but who knows what it might do to you?"));
            this.sweet = sweet;
        }
        use(character) {
            ...
            // Consuming Text
            if (this.sweet)
                ContentView_1.CView.text("You wad up the sweet, pink gossamer and eat it, finding it to be delicious and chewy, almost like gum.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.");
            else if (!this.sweet)
                ContentView_1.CView.text("You wad up the sweet, black gossamer and eat it, finding it to be delicious and chewy, almost like licorice.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.");
            // *************
            // Stat Changes
            // *************
            // (If speed<70, increases speed)
            if (character.stats.spe < 70 && changes < changeLimit && SMath_1.randInt(3) === 0) {
            ...
            if (character.stats.spe > 80 && changes < changeLimit && SMath_1.randInt(3) === 0) {
            ...
            if (character.stats.str > 70 && SMath_1.randInt(3) === 0) {
                ...
                if (RaceScore_1.spiderRaceScore(character) < 4)
                    ContentView_1.CView.text("Wait, you're not a spider, that doesn't make any sense!");
                else
                    ContentView_1.CView.text("Well, maybe you should put your nice, heavy abdomen to work.");
            ...
            const spiderbutts = character.body.tails.filter(Tail_1.Tail.FilterType(Tail_1.TailType.SPIDER_ABDOMEN));
            ...
            if (character.body.vaginas.length > 0) {
                ...
                if (vagina.looseness > 1 && changes < changeLimit && SMath_1.randInt(3) === 0) {
                    ContentView_1.CView.text("\n\nWith a gasp, you feel your " + VaginaDescriptor_1.describeVagina(character, vagina) + " tightening, making you leak sticky girl-juice. After a few seconds, it stops, and you rub on your " + VaginaDescriptor_1.describeVagina(character, vagina) + " excitedly. You can't wait to try this out!");
            ...
            if (character.body.butt.looseness > 1 && changes < changeLimit && SMath_1.randInt(3) === 0) {
                ContentView_1.CView.text("\n\nYou let out a small cry as your " + ButtDescriptor_1.describeButthole(character.body.butt) + " shrinks, becoming smaller and tighter. When it's done, you feel much hornier and eager to stretch it out again.");
                character.stats.lib += 2;
                character.stats.lust += 25;
                changes++;
            ...
            if (character.body.cocks.length > 0 && changes < changeLimit && SMath_1.randInt(4) === 0) {
                // Use temp to see if any dicks can be thickened
                let cockGotThickened = false;
                ...
                if (cockGotThickened) {
                    ContentView_1.CView.text("\n\nYou can feel your " + CockDescriptor_1.describeCocksLight(character) + " filling out in your " + character.inventory.armor.displayName + ". Pulling ");
                    if (character.body.cocks.length === 1)
                        ContentView_1.CView.text("it");
                    else
                        ContentView_1.CView.text("them");
                    ContentView_1.CView.text(" out, you look closely.  ");
                    if (character.body.cocks.length === 1)
                        ContentView_1.CView.text("It's");
                    else
                        ContentView_1.CView.text("They're");
                    ContentView_1.CView.text(" definitely thicker.");
                    changes++;
                }
            }
            // [Increase to Breast Size] - up to Large DD
            if (character.body.chest.length > 0) {
                const smallestBreastRow = character.body.chest.sort(BreastRow_1.BreastRow.Smallest).get(0);
                if (smallestBreastRow.rating < 6 && changes < changeLimit && SMath_1.randInt(4) === 0) {
                    ContentView_1.CView.text("\n\nAfter eating it, your chest aches and tingles, and your hands reach up to scratch at it unthinkingly.  Silently, you hope that you aren't allergic to it.  Just as you start to scratch at your " + BreastDescriptor_1.describeBreastRow(smallestBreastRow) + ", your chest pushes out in slight but sudden growth.");
                    smallestBreastRow.rating++;
                    changes++;
                }
            }
            // [Increase to Ass Size] - to 11
            if (character.body.butt.rating < 11 && changes < changeLimit && SMath_1.randInt(4) === 0) {
                ContentView_1.CView.text("\n\nYou look over your shoulder at your " + ButtDescriptor_1.describeButt(character) + " only to see it expand just slightly. You gape in confusion before looking back at the remaining silk in your hands. You finish it anyway. Dammit!");
            ...
            if (character.body.ears.type !== Ears_1.EarType.HUMAN && character.body.ears.type !== Ears_1.EarType.ELFIN && SMath_1.randInt(4) === 0 && changes < changeLimit) {
            ...
            // (Fur/Scales fall out)
            if (character.body.skin.type !== Skin_1.SkinType.PLAIN && (character.body.ears.type === Ears_1.EarType.HUMAN || character.body.ears.type === Ears_1.EarType.ELFIN) && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ContentView_1.CView.text("\n\nA slowly-building itch spreads over your whole body, and as you idly scratch yourself, you find that your " + SkinDescriptor_1.skinFurScales(character) + " ");
                if (character.body.skin.type === Skin_1.SkinType.SCALES)
                    ContentView_1.CView.text("are");
                else
                    ContentView_1.CView.text("is");
                ContentView_1.CView.text(" falling to the ground, revealing flawless, almost pearly-white skin underneath.  <b>You now have pale white skin.</b>");
            ...
            // (Gain human face)
            if (character.body.skin.type === Skin_1.SkinType.PLAIN && (character.body.face.type !== Face_1.FaceType.SPIDER_FANGS && character.body.face.type !== Face_1.FaceType.HUMAN) && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            if (changes < changeLimit && character.body.chest.length > 2 && SMath_1.randInt(3) === 0 && !Settings_1.Settings.hyperHappy) {
                changes++;
            ...
            // Nipples Turn Black:
            if (!character.effects.has(EffectType_1.EffectType.BlackNipples) && SMath_1.randInt(6) === 0 && changes < changeLimit) {
                ...
                character.effects.create(EffectType_1.EffectType.BlackNipples);
                changes++;
            }
            // eyes!
            if (character.body.skin.type === Skin_1.SkinType.PLAIN && character.body.face.type !== Face_1.FaceType.SPIDER_FANGS && character.body.face.type !== Face_1.FaceType.HUMAN && character.body.eyes.type === Eyes_1.EyeType.HUMAN && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                character.body.eyes.type = Eyes_1.EyeType.FOUR_SPIDER_EYES;
                changes++;
            ...
            // (Gain spider fangs)
            if (character.body.face.type === Face_1.FaceType.HUMAN && character.body.skin.type === Skin_1.SkinType.PLAIN && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            if (character.body.arms.type !== Arms_1.ArmType.SPIDER && changes < changeLimit && SMath_1.randInt(4) === 0) {
                ContentView_1.CView.text("\n\n");
                // (Bird pretext)
                if (character.body.arms.type === Arms_1.ArmType.HARPY)
                    ContentView_1.CView.text("The feathers covering your arms fall away, leaving them to return to a far more human appearance.  ");
                ContentView_1.CView.text("You watch, spellbound, while your forearms gradually become shiny.  The entire outer structure of your arms tingles while it divides into segments, turning the " + SkinDescriptor_1.skinFurScales(character) + " into a shiny black carapace.  You touch the onyx exoskeleton and discover to your delight that you can still feel through it as naturally as your own skin.");
                character.body.arms.type = Arms_1.ArmType.SPIDER;
                changes++;
            }
            // (Centaurs -> Normal Human Legs) (copy from elsewhere)
            if (character.body.legs.isTaur() && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            if (character.body.legs.isGoo() && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            if (character.body.legs.isNaga() && changes < changeLimit && SMath_1.randInt(4) === 0) {
            ...
            // Drider butt
            if (!this.sweet && !character.effects.has(EffectType_1.EffectType.SpiderOvipositor) && character.body.legs.isDrider() && character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.SPIDER_ABDOMEN), false) && changes < changeLimit && SMath_1.randInt(3) === 0 && (character.body.vaginas.length > 0 || SMath_1.randInt(2) === 0)) {
                ...
                // V1 - Egg Count
                // V2 - Fertilized Count
                character.effects.create(EffectType_1.EffectType.SpiderOvipositor);
            ...
            if (((!this.sweet && character.body.legs.type !== Legs_1.LegType.DRIDER_LOWER_BODY && character.body.legs.type !== Legs_1.LegType.CHITINOUS_SPIDER_LEGS) ||
                (this.sweet && character.body.legs.type !== Legs_1.LegType.CHITINOUS_SPIDER_LEGS)) &&
                (!character.body.legs.isGoo() && !character.body.legs.isNaga() && !character.body.legs.isTaur()) &&
                changes < changeLimit &&
                SMath_1.randInt(4) === 0) {
                ContentView_1.CView.text("\n\nStarting at your " + LegDescriptor_1.describeFeet(character) + ", a tingle runs up your " + LegDescriptor_1.describeLegs(character) + ", not stopping until it reaches your thighs.  From the waist down, your strength completely deserts you, leaving you to fall hard on your " + ButtDescriptor_1.describeButt(character) + " in the dirt.  With nothing else to do, you look down, only to be mesmerized by the sight of black exoskeleton creeping up a perfectly human-looking calf.  It crests up your knee to envelop the joint in a many-faceted onyx coating.  Then, it resumes its slow upward crawl, not stopping until it has girded your thighs in glittery, midnight exoskeleton.  From a distance it would look almost like a black, thigh-high boot, but you know the truth.  <b>You now have human-like legs covered in a black, arachnid exoskeleton.</b>");
                character.body.legs.type = Legs_1.LegType.CHITINOUS_SPIDER_LEGS;
                changes++;
            }
            // (Tail becomes spider abdomen GRANT WEB ATTACK)
            if (!character.body.tails.reduce(Tail_1.Tail.HasType(Tail_1.TailType.SPIDER_ABDOMEN), false) && (character.body.legs.type === Legs_1.LegType.CHITINOUS_SPIDER_LEGS || character.body.legs.type === Legs_1.LegType.DRIDER_LOWER_BODY) && character.body.arms.type === Arms_1.ArmType.SPIDER && SMath_1.randInt(4) === 0) {
                ContentView_1.CView.text("\n\n");
                // (Pre-existing tails)
                if (character.body.tails.length > 0)
                    ContentView_1.CView.text("Your tail shudders as heat races through it, twitching violently until it feels almost as if it's on fire.  You jump from the pain at your " + ButtDescriptor_1.describeButt(character) + " and grab at it with your hands.  It's huge... and you can feel it hardening under your touches, firming up until the whole tail has become rock-hard and spherical in shape.  The heat fades, leaving behind a gentle warmth, and you realize your tail has become a spider's abdomen!  With one experimental clench, you even discover that it can shoot webs from some of its spinnerets, both sticky and non-adhesive ones.  That may prove useful.  <b>You now have a spider's abdomen hanging from above your " + ButtDescriptor_1.describeButt(character) + "!</b>\n\n");
                // (No tail)
                else
                    ContentView_1.CView.text("A burst of pain hits you just above your " + ButtDescriptor_1.describeButt(character) + ", coupled with a sensation of burning heat and pressure.  You can feel your " + SkinDescriptor_1.skinFurScales(character) + " tearing as something forces its way out of your body.  Reaching back, you grab at it with your hands.  It's huge... and you can feel it hardening under your touches, firming up until the whole tail has become rock-hard and spherical in shape.  The heat fades, leaving behind a gentle warmth, and you realize your tail has become a spider's abdomen!  With one experimental clench, you even discover that it can shoot webs from some of its spinnerets, both sticky and non-adhesive ones.  That may prove useful.  <b>You now have a spider's abdomen hanging from above your " + ButtDescriptor_1.describeButt(character) + "!</b>");
                character.body.tails.clear();
                const newTail = new Tail_1.Tail(Tail_1.TailType.SPIDER_ABDOMEN);
                newTail.venom = 5;
                newTail.recharge = 5;
    ...
    exports.SweetGossamer = SweetGossamer;
});
//# sourceMappingURL=SweetGossamer.js.map