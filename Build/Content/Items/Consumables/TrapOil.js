define(["require", "exports", "Engine/Items/Consumable", "../ConsumableName", "Engine/Utilities/SMath", "Engine/Body/BreastRow", "Engine/Body/Cock", "Engine/Body/Eyes", "Engine/Body/Vagina", "Engine/Body/Wings", "Content/Effects/EffectType", "Content/Utilities/NumToText", "Engine/Items/ItemDesc", "Content/Descriptors/CockDescriptor", "Engine/Body/GenderIdentity", "Content/Descriptors/LegDescriptor", "Engine/Display/ContentView", "Content/Modifiers/BodyModifier", "Content/Modifiers/CockModifier"], function (require, exports, Consumable_1, ConsumableName_1, SMath_1, BreastRow_1, Cock_1, Eyes_1, Vagina_1, Wings_1, EffectType_1, NumToText_1, ItemDesc_1, CockDescriptor_1, GenderIdentity_1, LegDescriptor_1, ContentView_1, BodyModifier_1, CockModifier_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class TrapOil extends Consumable_1.Consumable {
        constructor() {
            super(ConsumableName_1.ConsumableName.TrapOil, new ItemDesc_1.ItemDesc("TrapOil", "a vial of trap oil", "A round, opaque glass vial filled with a clear, viscous fluid.  It has a symbol inscribed on it, a circle with a cross and arrow pointing out of it in opposite directions.  It looks and smells entirely innocuous."));
        }
        use(character) {
            ...
            if (character.effects.has(EffectType_1.EffectType.HistoryAlchemist))
                changeLimit++;
            ContentView_1.CView.text("You pour some of the oil onto your hands and ");
            if (character.stats.cor < 30)
                ContentView_1.CView.text("hesitantly ");
            else if (character.stats.cor > 70)
                ContentView_1.CView.text("eagerly ");
            ContentView_1.CView.text("rub it into your arms and chest.  The substance is warm, coating and ever so slightly numbing; it quickly sinks into your skin, leaving you feeling smooth and sleek.");
            // Speed Increase:
            if (character.stats.spe < 100 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
            ...
            // Strength Loss:
            else if (character.stats.str > 40 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
            ...
            if (character.stats.sens < 70 && character.body.cocks.length > 0 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
                ContentView_1.CView.text("\n\nA light breeze brushes over you and your skin tingles.  You have become more sensitive to physical sensation.");
                character.stats.sens += 5;
                changes++;
            }
            // Libido Increase:
            if (character.stats.lib < 70 && character.body.vaginas.length > 0 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
            ...
            // Body Mass Loss:
            if (character.body.thickness > 40 && SMath_1.randInt(3) === 0 && changes < changeLimit) {
                ...
                BodyModifier_1.displayModThickness(character, 40, 3);
                changes++;
            }
            // Thigh Loss: (towards �girly�)
            if (character.body.hips.rating >= 10 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                if (character.body.hips.rating > 15)
                    character.body.hips.rating -= 2 + SMath_1.randInt(3);
                changes++;
            }
            // Thigh Gain: (towards �girly�)
            if (character.body.hips.rating < 6 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
            ...
            if (character.body.chest.length > 0) {
                ...
                if (character.body.chest.sort(BreastRow_1.BreastRow.Largest).get(0).rating > 1 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                    ...
                    for (const breastRow of character.body.chest) {
                        if (breastRow.rating > 70)
                            breastRow.rating -= SMath_1.randInt(3) + 15;
                        else if (breastRow.rating > 50)
                            breastRow.rating -= SMath_1.randInt(3) + 10;
                        else if (breastRow.rating > 30)
                            breastRow.rating -= SMath_1.randInt(3) + 7;
                        else if (breastRow.rating > 15)
                            breastRow.rating -= SMath_1.randInt(3) + 4;
                        else
                            breastRow.rating -= 2 + SMath_1.randInt(2);
                ...
                if (character.body.chest.sort(BreastRow_1.BreastRow.Largest).get(0).rating < 1 || character.body.chest.firstRow.rating < 1 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
            ...
            if (character.body.cocks.length > 0 && character.body.cocks.sort(Cock_1.Cock.Longest).get(0).length >= 3.5 && character.body.cocks.length > 0 && SMath_1.randInt(2) === 0 && changes < changeLimit) {
                ContentView_1.CView.text("\n\nYou flinch and gasp as your " + CockDescriptor_1.describeCocksLight(character) + " suddenly become");
                if (character.body.cocks.length === 1)
                    ContentView_1.CView.text("s");
                ContentView_1.CView.text(" incredibly sensitive and retract into your body.  Anxiously you pull down your underclothes to examine your nether regions.  To your relief ");
                if (character.body.cocks.length === 1)
                    ContentView_1.CView.text("it is");
                else
                    ContentView_1.CView.text("they are");
                ContentView_1.CView.text(" still present, and as you touch ");
                if (character.body.cocks.length === 1)
                    ContentView_1.CView.text("it");
                else
                    ContentView_1.CView.text("them");
                ContentView_1.CView.text(", the sensitivity fades, however - a blush comes to your cheeks - ");
                if (character.body.cocks.length === 1)
                    ContentView_1.CView.text("it seems");
                else
                    ContentView_1.CView.text("they seem");
                ContentView_1.CView.text(" to have become smaller.");
                for (const cock of character.body.cocks) {
                    if (cock.length >= 3.5) {
                        // Shrink said cock
                        if (cock.length < 6 && cock.length >= 2.9) {
                            cock.length -= .5;
                            if (cock.thickness * 6 > cock.length)
                                cock.thickness -= .2;
                            if (cock.thickness * 8 > cock.length)
                                cock.thickness -= .2;
                            if (cock.thickness < .5)
                                cock.thickness = .5;
                        }
                        cock.length -= 0.5;
                        CockModifier_1.growCock(character, cock, Math.round(cock.length * 0.33) * -1);
                    }
                }
                changes++;
            }
            // Testicle Reduction:
            if (character.body.balls.count > 0 && character.body.cocks.length > 0 && (character.body.balls.size > 1 || !character.effects.has(EffectType_1.EffectType.Uniball)) && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                if (character.body.balls.size > 8)
                    character.body.balls.size--;
                ...
                if (character.body.balls.size < 1 && !character.effects.has(EffectType_1.EffectType.Uniball)) {
                    ContentView_1.CView.text("  You whimper as once again, your balls tighten and shrink.  Your eyes widen when you feel the gentle weight of your testicles pushing against the top of your [hips], and a few hesitant swings of your rear confirm what you can feel - you've tightened your balls up so much they no longer hang beneath your " + CockDescriptor_1.describeCocksLight(character) + ", but press perkily upwards.  Heat ringing your ears, you explore your new sack with a careful hand.  You are deeply grateful you apparently haven't reversed puberty, but you discover that though you still have " + NumToText_1.numToCardinalText(character.body.balls.count) + ", your balls now look and feel like one: one cute, tight little sissy parcel, its warm, insistent pressure upwards upon the joining of your thighs a never-ending reminder of it.");
                    ...
                    character.effects.create(EffectType_1.EffectType.Uniball);
                }
            ...
            if (character.body.butt.wetness < 5 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                if (character.body.butt.wetness < 4)
            ...
            // Fertility Decrease:
            if (character.body.vaginas.length > 0 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                character.stats.sens += -2;
                // High fertility:
                if (character.body.fertility >= 30)
                    ContentView_1.CView.text("It feels like your overcharged reproductive organs have simmered down a bit.");
                // Average fertility:
                else if (character.body.fertility >= 5)
                    ContentView_1.CView.text("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.");
                // [Low/No fertility:
                else {
                    ContentView_1.CView.text("Although the numbness makes you feel serene, the trap oil has no effect upon your ");
                    if (character.body.fertility > 0)
                        ContentView_1.CView.text("mostly ");
                    ContentView_1.CView.text("sterile system.");
                    // [Low/No fertility + Trap/Corruption  >70:
                    if (character.stats.cor > 70)
                        ContentView_1.CView.text("  For some reason the fact that you cannot  as nature intended makes you feel helpless and submissive.  Perhaps the only way to be a useful creature now is to find a dominant, fertile being willing to plow you full of eggs? You shake the alien, yet oddly alluring thought away.");
                }
                character.body.fertility -= 1 + SMath_1.randInt(3);
                if (character.body.fertility < 4)
                    character.body.fertility = 4;
                changes++;
            }
            // Male Effects
            if (character.gender === GenderIdentity_1.Gender.MALE) {
                ...
                if (SMath_1.randInt(4) === 0 && changes < changeLimit) {
                    if (character.body.femininity < 70 && character.body.femininity >= 60) {
                        ...
                        if (!character.effects.has(EffectType_1.EffectType.Androgyny)) {
                            character.effects.create(EffectType_1.EffectType.Androgyny);
                ...
                if (character.body.tone > 20 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
            ...
            // Female Effects
                ...
                if (character.body.femininity > 30 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                    character.body.femininity -= 10;
                ...
                // Muscle tone gain:
                if (character.body.tone < 80 && SMath_1.randInt(4) === 0 && changes < changeLimit) {
            ...
            // Remove odd eyes
            if (character.body.eyes.type === Eyes_1.EyeType.FOUR_SPIDER_EYES && SMath_1.randInt(2) === 0 && changes < changeLimit) {
            ...
            // PC Trap Effects
            if (character.body.eyes.type !== Eyes_1.EyeType.BLACK_EYES_SAND_TRAP && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                character.body.eyes.type = Eyes_1.EyeType.BLACK_EYES_SAND_TRAP;
                // Eyes Turn Black:
            ...
            // Vagina Turns Black:
            if (character.body.vaginas.length > 0 && character.body.vaginas.get(0).type !== Vagina_1.VaginaType.BLACK_SAND_TRAP && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                if (character.body.vaginas.get(0).wetness >= 3)
                    ContentView_1.CView.text("  Your natural lubrication makes it gleam invitingly.");
                // (Corruption <50:
                if (character.stats.cor < 50)
                    ContentView_1.CView.text("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.");
                else
                    ContentView_1.CView.text("  After a few cautious touches you decide it doesn't feel any different - the sheer bizarreness of it is a big turn on though, and you feel it beginning to shine with anticipation at the thought of using it.");
                ContentView_1.CView.text("  <b>Your vagina is now ebony in color.</b>");
                character.stats.sens += 2;
                character.stats.lust += 10;
                character.body.vaginas.get(0).type = Vagina_1.VaginaType.BLACK_SAND_TRAP;
                changes++;
            }
            // Dragonfly Wings:
            if (character.body.wings.type !== Wings_1.WingType.GIANT_DRAGONFLY && SMath_1.randInt(4) === 0 && changes < changeLimit) {
                ...
                changes++;
                character.body.wings.type = Wings_1.WingType.GIANT_DRAGONFLY;
            }
            if (changes === 0) {
    ...
    exports.TrapOil = TrapOil;
});
//# sourceMappingURL=TrapOil.js.map